...
\begin{center}
\vskip 1ex
\centerline{\psfig{file=dlpoly_files.eps,height=12cm}}
...
\D requires seven input files named CONTROL, CONFIG, FIELD, TABLE,
TABEAM, REFERENCE and REVOLD.  The first three files are mandatory,
whereas TABLE and TABEAM are only used to input certain kinds of
pair or metal potentials, and may not always be required.  REFERENCE
is required only if defect detection is switched on in CONTROL.
...
io read  mpiio        2 50000 5000

# I/O WRITE: METHOD, TYPE, WRITER COUNT, BATCH & BUFFER SIZES
io write mpiio sorted 8 50000 5000
...
\>                                              \> \\
\> {\bf cut}off $f$                             \> set required long-ranged interactions\index{potential!electrostatics} cutoff, $r_{\rm cut}$, to $f$ \AA~ \\
\>                                              \> \\
\> {\bf defe}cts $i~j~f$                        \> write DEFECTS file with controls: \\
\>                                              \> $i~=$ start timestep for dumping defects configurations (default $i~=~0$) \\
...
\>                                              \> ${\rm Min}\left[0.3,r_{\rm cut}/3\right]$~\AA~$\le~f~\le~{\rm Min}\left[1.2,r_{\rm cut}/2\right]$~\AA) \\
\>                                              \> \\
...
\>                                              \> \\
\> {\bf ensemble nst lang}evin $f_{1}~f_{2}$    \> select N$\mat{\sigma}$T ensemble\index{ensemble!Langevin N$\mat{\sigma}$T}, type Langevin with $f_{1},~f_{2}$ as the \\
\>                                              \> thermostat\index{thermostat} and barostat\index{barostat} relaxation speed (friction) constants in ps$^{-1}$ \\
\>                                              \> \\
...
\>                                              \> \phantom{xxx} {\bf \em k, batch size} :: $1~\le~k~\le~1,000,000$ (default $50,000$) \\
...
\>                                              \> {\bf \em l, buffer size} :: $100~\le~l~\le~100,000$ (default $5,000$) \\
...
\>                                              \> alloy systems using the EAM (TABEAM) \\
\>                                              \> \\
...
\>                                              \> matches the crystalographic sites from the CONFIG file by \\
\>                                              \> their order of reading rather than by their actual indexing \\
\>                                              \> \\
\> {\bf no str}ict                              \> {\bf (i)} abort strict checks such as; on existance of well defined \\
...
\>                                              \> be {\em force} ($1~\le~f~\le~1000$, default $f~=~100$) or {\em energy} \\
...
\>                                              \> {\em string}, where string can only be {\em langevin} or {\em direct} \\
\>                                              \> (if neither is specified both are applied in order \\
\>                                              \> {\em langevin$\rightarrow$direct}), $f_{1}$ is the thickness of the thermostat \\
...
\>                                              \> with Fennell \cite{fennell-06} damping (Ewald-like convergence) \\
...
\>                                              \> with Fennell \cite{fennell-06} damping (Ewald-like) automatic parameter \\
\>                                              \> optimisation ($10^{-20}~\le~f~\le~0.5$, default $f~=~10^{-20}$) \\
\>                                              \> \\
...
\>                                              \> \\
\> {\bf replay}                                 \> abort simulation and replay HISTORY to recalculate structural \\
\>                                              \> properties such as RDFs, z-density profiles, defects detection \\
\>                                              \> (execution holts if no property is specified) \\
\>                                              \> \\
...
(not mutually exclusive but if {\bf temp} has to preceed
...
optimiser at the beginning of the equlibration period or when
...
option is used in conjunction with the default values of {\bf
maxdis} ($0.03$~\AA) and {\bf mindis} ($0.10$~\AA), which can also
be optionally altered if used as directives (note the rule that {\bf
maxdis}~$>~2.5$~{\bf mindis} applies).  These distances serve as
...
doubled and the step repeated.  In this way the integration timestep
self-adjusts in response to the dynamics of the system.
...
control file, the simulation will start as new.  When specifed, it
...
30 & Langevin N$\mat{\sigma}$T\index{ensemble!Langevin N$\mat{\sigma}$T} ensemble \\
...
optimisation.  The target temperature of the simulation is reset to
$10$ Kelvin and a crude energy minimiser:
\begin{equation}
\vek{v}_{i} \leftarrow \left\{ \begin{array} {l@{\qquad:\qquad}l}
...
\end{array} \right.
\end{equation}
is used to help the system relax before each integration of the
equations of motion (measures are taken to conserve the MD cell
momentum).  This must not be thought of as a true energy
minimization method.  {\bf Note} that this optimisation is applied
irrespectively of whether the simulation runs in equilibration or
statistical mode.
...
initiated after any possible equlibration.  Otherwise, the
system will be thermostated and the impact energy disipated
during the equlibration.
...
algorithms - {\bf(i)} Langevin (stochastic thermostat) and {\bf(ii)}
Direct (direct thermostat).  If no type is specified then the
Langevin temperature control algorithm is applied first followed the
...
{\bf ( i)} holt if the simulation has been restarted, i.e. is a
...
will holt).  If a particle, $p$, is located in the vicinity of a
...
mxcons, mxteth, mxbond, mxangl, mxdihd, mxinv}) as well as the
lengths of link-cell ({\tt mxlist}) and domain ({\tt mxatms})
...
strictly mandatory, any other items are not read by \D but may be
added to aid alternative uses of the file, for example the \D
Graphical User Interface\index{Graphical User
...
descriptions, and non-bonded\index{potential!non-bonded} interaction descriptions, appearing in that
order in the file.
...
\begin{description}
\item[a.] {\bf eV}, for electron-volts per mol
\item[b.] {\bf kcal}, for k-calories per mol
\item[c.] {\bf kJ}, for k-Joules per mol
\item[d.] {\bf K}elvin, for Kelvin per mol
...
{\bf pmf}, {\bf rigid}, {\b teth}, {\bf bonds}, {\bf angles},
...
{\bf -msb} &                     &     &              &            &            & \\
           &                     &     &              &            &            & \\
{\bf mmsb} & Compass \cite{sun-98} & $A$ & $r_{ij}^{o}$ & $r_{jk}^{o}$ &        &
$U(\theta) = A~(r_{ij} - r_{ij}^{o})~(r_{ik} - r_{ik}^{o})$ \\
{\bf -msb} & stretch-stretch     &     &              &            &            & \\
           &                     &     &              &            &            & \\
{\bf mmsb} & Compass \cite{sun-98} & $A$ & $\theta_{0}$ & $r_{ij}^{o}$ &        &
$U(\theta) = A~(\theta-\theta_{0})~(r_{ij} - r_{ij}^{o})$ \\
{\bf -msb} & stretch-bend        &     &              &            &            & \\
           &                     &     &              &            &            & \\
{\bf mmsb} & Compass \cite{sun-98} & $A$ & $B$       & $C$       & $\theta_{0}$ &
$U(\theta) = A~(r_{ij} - r_{ij}^{o})~(r_{ik} - r_{ik}^{o}) + (\theta-\theta_{0}) \times $ \\
{\bf -msb} & all terms           & $r_{ij}^{o}$ & $r_{jk}^{o}$ &   &            &
...
do not contribute to the {\em excluded atoms list} (see Section
\ref{force-field}).  In this case \D will calculate the
...
$U(r) = E_{0}[\{1-\exp(-k(r-r_{0}))\}^{2}-1]$ \\
           &                           &            &          &          &         &                      & \\
{\bf wca}  & Shifted$^{*}$ Weeks-      & $\epsilon$ & $\sigma$ & $\Delta$ &         &                      &
...
By default in DL\_POLY\_2 and \D every {\bf vdw} and {\bf met}
...
that will not be used.  (This option is not available in DL\_POLY\_2.)

{\bf Note} that {\bf rdf} and {\bf vdw/met} are not complementary -
i.e. if the former is used none of the pairs defined by the latter
will be considered for RDF calculations.

The selected RDFs are calculated in the {\sc rdf\_collect} by
collecting distance information from all two-body pairs as encountered
...
\hline
key & potential type & \multicolumn{6}{c|}{Variables (1-5,6-11,a-c)} & functional form \\
\hline
           &          &        &          &        &     &     &     & \\
...
body\index{potential!three-body} potential is truncated.  The
...
four-body\index{potential!four-body} potential is truncated.  The
distance is in \AA, measured from the central atom.
...
by the field parameters. The variables pertaining to each field
potential are described in Table \ref{external-field-table}.
...
by the the nearest real number.  The contents are as follows (the
...
{\bf record 2} \\
\> {\tt delpot} \> real    \> mesh resolution in \AA~(${\tt delport} = \frac{\tt cutpot}{{\tt ngrid}-4}$) \\
...
analytic form) describing the EAM\index{potential!EAM} metal
...
$n(n+1)/2$ {\em cross} pair potential functions.  This makes $n(n+5)/2$
functions in total.  {\bf Note} that the option of using EAM\index{potential!EAM}
interactions must also be {\em explicitly} declared in the FIELD file
so that for the $n$ component alloy there are $n(n+1)/2$ {\em cross} pair
...
type) - {\bf embe}ding keyword, and $n(n+1)/2$ {\em cross} pair potential
...
followed by predefined number of data records {\bf as a maximum of four data
...
\>               \>         \> or in density units for {\bf embe}d \\
{\bf funtion data records:} ({\em number of data records} $=$ {\tt Int}(({\tt ngrid}+3)/4)) \\
\> {\tt data 1}  \> real    \> data item 1 \\
\> {\tt data 2}  \> real    \> data item 2 \\
\> {\tt data 3}  \> real    \> data item 3 \\
\> {\tt data 4}  \> real    \> data item 4 \\
...
The tabled data are used to fill the internal arrays {\tt gmet},
{\tt fmet} and {\tt vmet} respectively (see Section
\ref{metal}).  The force arrays are generated from these (by the
{\sc metal\_table\_derivatives} routine) using a five point
interpolation precedure.  During simulation, interactions beyond
distance $Min(r_{\rm cut},{\tt limit 2})$ are discarded, whereas
interactions at distances shorter than {\tt limit 1} will cause the
simulation to abort.  The simulation will also abort if any local
density exceeds the limits for the embedding function.
