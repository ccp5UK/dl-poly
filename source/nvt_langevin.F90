...
Contains
  ...
  Subroutine nvt_l0_vv(stage, lvar, mndis, mxdis, mxstp, tstep, nstep, strkin, engke, &
    ...
    If (stage == VV_FIRST_STAGE) Then
      ...
      Allocate (fxr(1:config%nlast), fyr(1:config%nlast), fzr(1:config%nlast), Stat=fail(1))
      Allocate (fxl(1:config%nlast), fyl(1:config%nlast), fzl(1:config%nlast), Stat=fail(2))
      If (Any(fail > 0)) Then
        Write (message, '(a)') 'nvt_l0 allocation failure+'
        Call error(0, message)
      End If
