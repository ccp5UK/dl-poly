!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  MD_LFV INCLUSION MODULE  !!!!!!!!!!!!!!!


! forces are always calculated at restart for LFV
...
! Calculate kinetic stress tensor and energy
...
! Langevin thermostat (Stochastic Dynamics)
              ...
              Call nvt_l0_lfv                             &
           (lvar,mndis,mxdis,temp,tstep,chi,strkin,engke, &
           imcon,mxshak,tolnce,megcon,strcon,vircon,      &
           megpmf,strpmf,virpmf)
...
! Save restart data in event of system crash
        ...
        If (Mod(nstep,ndump) == 0 .and. nstep /= nstrun) Call system_revive &
