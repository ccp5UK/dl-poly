...
+ new potentials
  ...
  + Rydberg
  + modified LJ
+ rdf can now calculate error bars
+ nfold now deals correctly with cubic cells
...
+ enhance STATIS writing thanks to Mike Pacey, University of Lancaster (UK)
+ windows cross-compilation from linux, both serial and parallel
+ fixes to:
  + restart for velocity autocorrelation functions
  + bugs in trajectory writing
  + reading from tables for bonds, angles and dihedrals, thanks to Tom Potter from University of Durham (UK)
  + bug in units for TABLE file
