...
! truncated octahedral boundary conditions
     ...
     If (.not.(Abs(cell(1)-cell(5)) < 1.0e-6_wp .and. Abs(cell(5)-cell(9)) < 1.0e-6_wp)) Then
        Write(*,*) 'Error in IMAGES'
        STOP
     End If
...
! rhombic Dodecahedral boundary conditions
     ...
     If (.not.(Abs(cell(1)-cell(5)) < 1.0e-6_wp .and. Abs(cell(9)-cell(1)*rt2) < 1.0e-6_wp)) Then
        Write(*,*) 'Error in IMAGES'
        STOP
     End If
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  If (newjob) Then
     ...
     to = Real(value(5),wp)*3600.0_wp + Real(value(6),wp)*60.0_wp   + &
...
! are assumed to be shorter than 24h
     ...
     t = Real(value(5),wp)*3600.0_wp + Real(value(6),wp)*60.0_wp   + &
         Real(value(7),wp)           + Real(value(8),wp)/1000.0_wp + &
         Real(days,wp)*86400.0_wp - to
  End If
