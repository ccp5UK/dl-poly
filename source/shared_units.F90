...
Contains
  ...
  Subroutine pass_shared_units(config, mx_u, b_l, b_u, nt_u, list_u, mxf_u, leg_u, lshmv, &
    ...
    Integer,                     Intent(In   ) :: mx_u, b_l, b_u
    Integer,                     Intent(InOut) :: nt_u, list_u(b_l:b_u, 1:mx_u)
    Integer,                     Intent(In   ) :: mxf_u
    Integer,                     Intent(InOut) :: leg_u(0:mxf_u, 1:config%mxatdm)
    Logical,                     Intent(  Out) :: lshmv
    Integer,                     Intent(  Out) :: lishp(:)
    Logical,                     Intent(InOut) :: oldjob
    Type(domains_type),          Intent(In   ) :: domain
    Integer,                     Intent(  Out) :: lashp(1:domain%neighbours)
  ...
  Subroutine update_shared_units_parts(config, lishp, lashp, subtype, domain, comm)
    ...
    Type(configuration_type), Intent(InOut) :: config
    Integer,                  Intent(In   ) :: lishp(:), subtype
    Type(domains_type),       Intent(In   ) :: domain
