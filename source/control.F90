...
Contains
  ...
  Subroutine read_control(lfce, impa, ttm, dfcts, rigid, &
    ...
    Type&
      (inversions_type), &
      Intent(InOut)        :: inversion
    Type(z_density_type), &
      Intent(InOut)        :: zdensity
    Type&
      (neighbours_type), &
      Intent(InOut)        :: neigh
    Type(vdw_type), &
  ...
  Subroutine scan_control(rcter, max_rigid, imcon, imc_n, cell, xhi, yhi, zhi, mxgana, &
    ...
    Real(Kind=wp), &
      Intent(InOut)        :: cell(1:9)
    Real(Kind=wp), &
      Intent(In)           :: xhi, yhi, zhi
    ...
    Real(Kind=wp), &
      Parameter            :: rbin_def = 0.05_wp, rcbnd_def = 2.5_wp, rcut_def = 1.0_wp
...
! read termination
  ...
  Subroutine scan_control_io(io, netcdf, files, comm)
    ...
    Integer                :: err_r, io_read, io_write, itmp
