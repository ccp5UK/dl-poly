...
Contains
  ...
  Subroutine warning_special(kode, a, b, c, triggered, level_once, level_always)
    ...
    If (eworld%idnode == 0) Then
      ...
      Write (ounit, '(/,1x,a,i0)') '# warning issued ', kode
  ...
  Subroutine error(kode, message, master_only)
    ...
    If (eworld%idnode == 0) Then
      ...
      Case (1091)
        ...
        Write (ounit, '(/,1x,a)') '# error - too many atoms would be generated by system expand, max is 2,147,483,647'

      Case (1092)

        Write (ounit, '(/,1x,a)') '# error - too many atoms in CONFIG, max is 2,147,483,647'
