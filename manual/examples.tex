...
\subsection{Test Case 47 and 48: Hexane melt, atomistic and coarsed grained}

TEST47 contains a Hexane melt (1000 molecules) modelled by the OPLSAA
force-field (FF).  TEST48 represents the same system, which is now CG-mapped
...
This test case is to exemplify the Coarse-Graining (CG) procedure
...
The test is one of the tutorial cases from VOTCA package \cite{ruhle-09a}.
Therefore, the CONFIG, CONTROL and FIELD files are fully consistent
with the corresponding setup files found in the tutorial directory
``csg-tutorials/hexane'' of VOTCA.
