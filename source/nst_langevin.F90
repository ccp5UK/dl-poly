Module nst_langevin
  ...
  Use nst_nose_hoover, Only: nst_h0_scl,&
                             nst_h1_scl
  Use numerics,        Only: box_mueller_saru6,&
                             dcell,&
                             images,&
                             mat_mul,&
                             seed_type
  ...
  Use thermostat,      Only: CONSTRAINT_SEMI_ORTHORHOMBIC,&
...
Contains
  ...
  Subroutine nst_l0_vv(stage, lvar, mndis, mxdis, mxstp, tstep, nstep, degfre, stress, &
    ...
    Real(Kind=wp), Parameter :: uni1(1:9) = (/1.0_wp, 0.0_wp, 0.0_wp, 0.0_wp, 1.0_wp, 0.0_wp, &
                                0.0_wp, 0.0_wp, 1.0_wp/)

    Character(Len=256)         :: message
    Integer                    :: fail(1:9), i, iter
    Logical, Allocatable       :: lstitr(:)
    Real(Kind=wp)              :: aaa(1:9), bbb(1:9), cell0(1:9), celprp(1:10), engke0, eta0(1:9), &
                                  hstep, qstep, rstep, scale, str(1:9), str1(1:9), tmp, vir, &
                                  vom(1:3), vzero
...
! uni1 is the diagonal unit matrix
  ...
  Subroutine nst_l1_vv(stage, lvar, mndis, mxdis, mxstp, tstep, &
    ...
    Real(Kind=wp),            Intent(In   ) :: stress(1:9)
    Real(Kind=wp),            Intent(InOut) :: strkin(1:9), strknf(1:9), strknt(1:9), engke, engrot
    Real(Kind=wp),            Intent(  Out) :: consv
    ...
    Integer                    :: fail(1:14), i, i1, i2, irgd, iter, j, jrgd, krgd, lrgd, matms, &
                                  rgdtyp
    Logical, Allocatable       :: lstitr(:)
    Real(Kind=wp)              :: aaa(1:9), bbb(1:9), cell0(1:9), celprp(1:10), engke0, engknf, &
                                  engknt, engrot0, eta0(1:9), fmx, fmxl, fmy, fmyl, fmz, fmzl, &
                                  hstep, mxdr, opx, opy, opz, p0, p1, p2, p3, qstep, qt0, qt0l, &
                                  qt1, qt1l, qt2, qt2l, qt3, qt3l, rot(1:9), rstep, scale, &
                                  str(1:9), str1(1:9), tmp, tqx, tqxl, tqy, tqyl, tqz, tqzl, trx, &
                                  trxl, try, tryl, trz, trzl, vir, vom(1:3), vpx, vpy, vpz, vzero, &
                                  x(1:1), y(1:1), z(1:1)
