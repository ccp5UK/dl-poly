...
parameters.  These may be: electron volts ({\bf eV}); k-calories
({\bf kcal}); k-joules ({\bf kJ}); Kelvin ({\bf K}elvin); or the
\D internal units (10 J) ({\bf internal}).  There is no default
value.  Failure to specify any of these correctly, or reference
...
\subsubsection*{\underline{Message 45}: error - too many atoms in CONFIG file}

This should never happen!

\noindent \underline{\em Action}:

Recompile and resubmit.  Send the problem to us if this is
persistent.
...
Increase the parameter {\tt mxcell}.
...
planar and 3-dimensional rigid units.  There is the possibility
that the unit has all of its mass-bearing particles frozen while
some of the massless are not or the unit has just one mass-bearing
particle.  Check the definition of the rigid unit in the CONFIG file,
