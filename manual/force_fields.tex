...
\end{itemize}
However, bonded interactions such as chemical bonds, angles,
dihedrals and inversions can be defined over any possible mixture
...
\item umbrella sampling:  ({\bf upmf})
\begin{equation}
U_{AB} = \frac{k}{2} (R_{AB}-R_{0})^{2}~,
\end {equation}
is an umbrella sampling, harmonic potential with a force constant, $k$, and an equilibrium distance, $R_{0}$, acting between between the centres of masses of two molecules (non-overlapping clusters of particles), $A$ and $B$.
\end{enumerate}
...
umbrella sampling and Jarzynski-equation based steered MD.  The software,
