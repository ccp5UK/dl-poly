...
\>                                              \> $r_{\rm cut}$, to $f$ \AA~ ($0.2~\ge~f$, default $f~=~0$) \\
\>                                              \> \\
...
\>                                              \> \\
\> {\bf rcut} $f$  ($\equiv$ {\bf cut}off $f$)  \> set required long-ranged interactions\index{potential!electrostatics} cutoff, $r_{\rm cut}$, to $f$ \AA~ \\
\>                                              \> \\
\> {\bf rlxtol} $f$                             \> set tolerance for relaxed shell model to $f$ \\
\>                                              \> (default $f~=~1$ in D~\AA~~ps$^{-2}$) \\
\>                                              \> \\
\> {\bf rpad} $f$  ($\equiv$ {\bf pad}ding $f$) \> set optional padding to the major cutoff, \\
\>                                              \> $r_{\rm cut}$, to $f$ \AA~ ($0.2~\ge~f$, default $f~=~0$) \\
\>                                              \> \\
...
$r_{\rm pad}$, if larger than $0.2$~\AA to the major cutoff, $r_{\rm cut}$,
to construct a larger link-cell width, $r_{\rm lnk}=r_{\rm cut}+r_{\rm pad}$,
which will trigger a construction of a larger Verlet neighbour list (VNL)
while at the same time facilitate its conditional update, rather at every
timestpe.  The VNL conditaional update is check at the end of each tiemstep
and triggered only when the most travelled particle has moved a distance
larger than $r_{\rm pad}/2$.  It is worth noting that padding is at expense
of extra memory but if used wisely it could improve time to solution from
5\% to 150\% depending on force-field complexity.  If it is too large or too
small (that is why the $\ge~0.2$~\AA limit) it will lead to performace
degradation.  It is recomended that $r_{\rm pad}$ is set up at a value of
$\approx~2~\div~10\%$ of the cutoff, $r_{\rm cut}$, as long as the major
link-cell algorithm has link-cell distribution not wiorse than
$4~\otimes~4~\otimes~4$ per domain.  For such setups, in practice, one
may expect speedups (in time to solution) of the order of $10~\div~30\%$
for force-fields involving the Ewald summation methodology and $30~\div~70\%$
for force-fields without electrostatics evaluations involving the Ewald
...
$\vek{F} = A \; (z_{0} - z) ~:~p . z > p . z_{0}$ \\
           &                     &       &           &         &         &               & \\
{\bf xpis} & X-Piston & $i^{glob}_{ind}$ & $j^{glob}_{ind}$ & $P^{in}_{k-atm}$ & &       &
$\vek{F}_{x} = \frac{\textrm{$P$} . \vek{\textrm{Area}}(\perp\textrm{X-}dir)}
{\left[\sum_{k=i}^{j}m_{k}\right] \textrm{/} m_{k}} : \forall~k=i,..,j $ \\
           &                     &       &           &         &         &               & \\
{\bf zres} & Molecule in HR Zone & $i^{glob}_{ind}$ & $j^{glob}_{ind}$ & $k$ & $z_{mn}$ & $z_{mx}$ &
$
...
$ \\
           &                     &       &           &         &         &               & \\
{\bf zrs-} & HR Zone (pull out)  & $i^{glob}_{ind}$ & $j^{glob}_{ind}$ & $k$ & $z_{mn}$ & $z_{mx}$ &
$
\vek{F}_{z} = \left\{ \begin{array} {l@{~:~}l}
...
$ \\
           &                     &       &           &         &         &               & \\
{\bf zrs+} & HR Zone (pull in)   & $i^{glob}_{ind}$ & $j^{glob}_{ind}$ & $k$ & $z_{mn}$ & $z_{mx}$ &
$
\vek{F}_{z} = \left\{ \begin{array} {l@{~:~}l}
A(z-z_{mx}) & z > z_{mx} \\
A(z_{mn}-z) & z < z_{mn}
...
$ \\
           &                     &       &           &         &         &               & \\
{\bf osel} & Osc. Electric Field & $E_{x}$ & $E_{y}$ & $E_{z}$ & $\omega$ &              &
...
\left[H\right] &=& \frac{\tt Dalton~\AA/ps^{2}}{\tt proton~\AA/ps} = \frac{\tt Dalton}{\tt proton~ps} \\
\left[H\right] &=& 1.037837512 \times 10^{4}~{\tt Tesla}~~. \nonumber
\end{eqnarray}
Thus to apply a magnetic field of 1~Tesla along the $y$ axis,
one could specify in FILED the following:
\begin{verbatim}
external
...
${\tt delpot} = \frac{\tt cutpot}{{\tt ngrid}-4}~<~{\tt dlrvdw} = \frac{\tt rvdw}{{\tt mxgrid}-4}$
(usually when ${\tt ngrid}>{\tt mxgrid}$), but will abort if ${\tt delpot}>{\tt dlrvdw}$.
