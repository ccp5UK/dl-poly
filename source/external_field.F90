...
Contains
  ...
  Subroutine external_field_apply(time, leql, nsteql, nstep, cshell, stats, rdf, &
    ...
    Integer                    :: fail(1:2), i, ia, ib, ic, id, irgd, j, jrgd, lrgd, rgdtyp
    Integer, Allocatable       :: lstopt(:, :)
    Logical                    :: l1, l2, safe
    Real(Kind=wp)              :: cm2(0:3), cmm(0:3), gamma, rrr, rtmp(1:2), rz, tmp, vxt, x(1:1), &
                                  y(1:1), z(1:1), zdif
...
! for oscillating fields
  ...
  Subroutine external_field_correct(engfld, ext_field, rigid, config, comm)
    ...
    Integer       :: i, ia, ib, irgd, j, jrgd, lrgd, rgdtyp
    Real(Kind=wp) :: rtmp(1:2), rz, tmp, vxt, x(1:1), y(1:1), z(1:1)
