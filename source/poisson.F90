...
Contains
  ...
  Subroutine poisson_forces(engcpe, vircpe, stress, pois, electro, domain, config, comm)
    ...
    Real(Kind=wp) :: eng, strs(1:9), virr
...
! copy DD mapping
  ...
  Subroutine biCGStab_charge_density(pois, electro, config, comm)
    ...
    Integer       :: i, j, k, n
    Real(Kind=wp) :: det, rcell(9), reps0dv, txx, tyy, tzz
...
!$OMP       END paralleldo
  ...
  Recursive Subroutine P_solver_omp(pois, comm)
    ...
    Integer           :: i, j, k, mmm
    Real(Kind=wp)     :: dphi, element, normphi0_local, normphi1_local, sm1, sm2, sm3, sm4, &
                         Totend, Totstart

!! SM stands for Stoyan Markov (long live!!)
...
!$OMP       END paralleldo
  ...
  Recursive Subroutine biCGStab_solver_omp(pois, comm)
    ...
    Character(Len=80) :: message
    Integer           :: kk, mmm
    Real(Kind=wp)     :: alfa, beta, normphi0_local, normphi1_local, omega, rho0, rho1, rv, &
                         Totend, Totstart, ts, tt
...
!$OMP       END paralleldo
  ...
  Contains
    ...
    Subroutine exchange_grid_halo(from, to, &
      ...
      !! This routine has no header !
    Integer,          Intent(In   ) :: from, to, xlb, xlt, ylb, ylt, zlb, zlt, xdb, xdt, ydb, ydt, &
                                       zdb, zdt
    Type(comms_type), Intent(InOut) :: comm
...
!$OMP     END paralleldo
  ...
  Subroutine poisson_excl_forces(iatm, rcut, eps, xxt, yyt, zzt, rrt, engcpe_ex, vircpe_ex, stress, neigh, config)
    ...
    Real(Kind=wp) :: chgea, chgprd, coul, fcoul, fix, fiy, fiz, fx, fy, fz, rrr, strs1, strs2, &
                     strs3, strs5, strs6, strs9
  ...
  Subroutine poisson_frzn_forces(eps, engcpe_fr, vircpe_fr, stress, neigh, config, comm)
    ...
    Real(Kind=wp)                            :: chgprd, coul, det, fcoul, fx, fy, fz, rcell(1:9), &
                                                rrr, rsq, strs1, strs2, strs3, strs5, strs6, &
                                                strs9, xrr, xss, yrr, yss, zrr, zss
    Real(Kind=wp), Allocatable, Dimension(:) :: cfr, rrt, xfr, xxt, yfr, yyt, zfr, zzt
