...
DL_POLY_5 is a distributed parallel molecular dynamics package based on
...
to add new features DL_POLY_5 will need to be familiar with the
...
processors. In DL_POLY_5 the domain allocation is handled by the routine
...
The DD strategy underpinning DL_POLY_5 is based on the link cell
...
to the interatomic potentials is relatively short ranged. In DL_POLY_5
...
This in DL_POLY_5 is handled by the ``set_halo_particles`` routine.
...
algorithm. In DL_POLY_5 this is accomplished quite naturally through the
...
``There are no load balancing algorithms in DL_POLY_5 to compensate for a bad density distribution.``
...
The intramolecular terms in DL_POLY_5 are managed through bookkeeping
...
Note that, at the start of a simulation DL_POLY_5 allocates individual
bonded interactions to specific processors, based on the domains of the
relevant atoms (DL_POLY_5 routine ``build_book_intra``). This means that
...
follows the integration of the equations of motion (DL_POLY_5 routine
...
DL_POLY_5 calculates the non-bonded pair interactions using the link
...
DL_POLY_5 routine ``build_excl_intra`` at the start of the simulation
...
during the atom relocation process described above (DL_POLY_5 routine
``exchange_particles``).
...
For systems with periodic boundary conditions DL_POLY_5 employs the
Ewald Sum to calculate the coulombic interactions (see
Section \ :ref:`SPME`). It should be noted that DL_POLY_5 uses
only the Smoothed Particle Mesh (SPME) form of the Ewald sum.

Calculation of the real space component in DL_POLY_5 employs the
...
entirely handled within the DL_POLY_5 by the 3D FFT routine
...
implicit in a distributed SPME application. The DL_POLY_5 subroutine
...
The simulation of metals (Section :ref:`metal`) by DL_POLY_5
...
distributed :index:`Verlet neighbour list` as outlined above. DL_POLY_5
...
DL_POLY_5 can calculate Tersoff, three-body and four-body interactions.
...
basis of the domain of the central atom in them. DL_POLY_5 implements
