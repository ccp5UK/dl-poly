...
\>                                              \> alloy systems using the EAM or EEAM (TABEAM) \\
\>                                              \> \\
\> {\bf metal sqrtrho}                          \> swiches the TABEAM default of reading embeding functions, \\
...
i.e. if the former is used in FILED none of the pairs defined by the
...
The EEAM potential for an $n$ component metal alloy is specified similarly
to the EAM potential, except that it requires the specification of $n^2$
electron density functions (one for each non-commuting pair of atom types).
This makes $3n(n+1)/2$ functions in total in TABEAM.  The option of using
EEAM\index{potential!EEAM} interactions must also be {\em explicitly}
declared in the FIELD file so that for the $n$ component alloy there are
$n(n+1)/2$ {\em cross} pair potential ({\bf eeam}) keyword entries in FIELD
despite that there must be $n^2$ pair density ({\bf eeam}) entries in TABEAM.
...
\end{tabbing}
For an $n$ component alloy, {\tt numpot} = $n(n+5)/2$ for EAM potential
and {\tt numpot} = $3n(n+1)/2$ for EEAM potential.

The subsequent records define the $n(n+5)/2$ functions for an $n$
component alloy - $n$ electron density functions for the EAM potential
form (one for each atom type) or $n^2$ for EEAM (one for each non-commuting
pair of atoms types in EEAM) - {\bf dens}ity keyword, $n$ embedding functions
(one for each atom type) - {\bf embe}ding keyword, and $n(n+1)/2$ {\em cross}
pair potential functions - {\bf pair}s keyword.  The functions may appear in
any random order in TABEAM as their identification is based on their unique
keyword, defined first in the function's header record.  The header record is
...
X\=XXXXXXXX\=XXXXXXXX\=\kill
{\bf header record:}\\
\> {\bf keyword} \> a4      \> type of EAM function: {\bf dens}ity, {\bf embe}d or {\bf pair} \\
\> {\tt atom 1}  \> a8      \> first atom type \\
\> {\tt atom 2}  \> a8      \> second atom type - only specified for {\bf pair} potential functions \\
\>               \>         \> and for {\bf dens}ity functions in the case of the EEAM potential\\
