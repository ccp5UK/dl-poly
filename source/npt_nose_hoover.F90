...
Contains
  ...
  Subroutine npt_h0_vv(stage, lvar, mndis, mxdis, mxstp, tstep, &
                       degfre, virtot, &
                       consv, &
                       strkin, engke, &
    ...
    Real(Kind=wp)              :: chip0, chit0, cint0, com(1:3), hstep, mxdr, qstep, rstep, scale, &
                                  str(1:9), tmp, vir, vom(1:3), vzero
    ...
    If (stage == VV_FIRST_STAGE) Then
      ...
      Do iter = 1, thermo%mxiter
        ...
        Do i = 1, config%natms
          If (config%weight(i) > 1.0e-6_wp) Then
            config%parts(i)%xxx = scale * (xxt(i) - com(1)) + tstep * config%vxx(i) + com(1)
            config%parts(i)%yyy = scale * (yyt(i) - com(2)) + tstep * config%vyy(i) + com(2)
            config%parts(i)%zzz = scale * (zzt(i) - com(3)) + tstep * config%vzz(i) + com(3)
          End If
        End Do
  ...
  Subroutine npt_h1_vv(stage, lvar, mndis, mxdis, mxstp, tstep, &
    ...
    Integer                    :: fail(1:14), i, i1, i2, irgd, iter, j, jrgd, krgd, lrgd, matms, &
                                  rgdtyp
    Logical, Allocatable       :: lstitr(:)
    Real(Kind=wp)              :: chip0, chit0, cint0, com(1:3), czero(1:9), fmx, fmy, fmz, hstep, &
                                  mxdr, opx, opy, opz, p0, p1, p2, p3, qstep, qt0, qt1, qt2, qt3, &
                                  rot(1:9), rstep, scale, str(1:9), tmp, tqx, tqy, tqz, trx, try, &
                                  trz, vir, vom(1:3), vpx, vpy, vpz, vzero, x(1:1), y(1:1), z(1:1)
            ...
            If (i <= config%natms) Then
              If (rigid%frozen(jrgd, rgdtyp) == 0) Then
                ...
                If (thermo%unsafe) Then
                  config%vxx(i) = scale * (xxt(i) - com(1))
                  config%vyy(i) = scale * (yyt(i) - com(2))
                  config%vzz(i) = scale * (zzt(i) - com(3))
  ...
  Subroutine npt_h0_scl(sw, tstep, degfre, chit, vir, virtot, engke, config, thermo)
    ...
    Integer(Kind=li),         Intent(In   ) :: degfre
    Real(Kind=wp),            Intent(In   ) :: chit, vir, virtot
  ...
  Subroutine npt_h1_scl(sw, tstep, degfre, degrot, chit, vir, virtot, vircom, engke, &
    ...
    Real(Kind=wp),            Intent(In   ) :: tstep
    Integer(Kind=li),         Intent(In   ) :: degfre, degrot
    Real(Kind=wp),            Intent(In   ) :: chit, vir, virtot, vircom
    Real(Kind=wp),            Intent(InOut) :: engke
    Type(rigid_bodies_type),  Intent(InOut) :: rigid
    Type(configuration_type), Intent(InOut) :: config
    Type(thermostat_type),    Intent(InOut) :: thermo
