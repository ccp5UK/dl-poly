...
%\title{Incorportating the two-temperature model \\ into DL\_POLY}
%\author{
%    Szymon Daraszewicz, Galvin Khara, \\
%	Department of Physics and Astronomy \\
%	University College London \\
%	\texttt{s.daraszewicz@gmail.com galvin.s.k@gmail.com} \\ \\
%}
%\usepackage{fancyhdr}
%\usepackage{cite}
%\usepackage{float}
%\usepackage{notoccite}
%\pagestyle{fancy} %change to "plain" if you get rid of fancyhdr
...
%\usepackage{gensymb}
%\usepackage{url}
%\usepackage[small, center, textfont=it, labelfont=bf]{caption} 
%\usepackage{subfig}

%\fancyhf{} % delete current header and footer
%\fancyhead[RO]{\bfseries\thepage} %LE = left of second page
%\fancyhead[LO]{\bfseries\rightmark} %right=subsection left=section
%\fancyhead[RE]{\bfseries\leftmark} - only applies to 2nd page of duplex
...
Various boundary condition choices are available for the edge cells 
...
ionic temperatures are only needed for finite-difference calculations of Equation 
\label{eq:electronic_system}, some of these communications only need to be 
applied in one direction for each dimension. 
...
density $\rho = \frac{N}{\Delta V}$, which is assumed to be constant 
throughout the system. Tabulated volumetric heat capacities are given 
...
The CONTROL file directive {\bf ttm diff} supplies this value in m$^2$~s$^{-1}$, 
which is subsequently converted to \AA$^2$~ps$^{-1}$.
...
considered inactive: the electronic temperature in the corresponding 
CET voxel is set to the system temperature and is excluded from 
thermal diffusion calculations. All voxels within the CIT grid are checked 
...
diffusion in the single MD timestep. This requires at least one electronic 
...
\begin{itemize}
	...
	\item  {\sc w\_integrate\_lfv.f90}: modified to call the new inhomogeneous Langevin thermostat ({\sc nvt\_l2\_lfv.f90} and {\sc nvt\_l2\_vv.f90});
