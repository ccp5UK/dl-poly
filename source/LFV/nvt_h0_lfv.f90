...
! molecular dynamics - leapfrog verlet with Hoover thermostat
!
! copyright - daresbury laboratory
! author    - i.t.todorov february 2004
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use setup_module
  Use config_module,      Only : natms,ltg,weight, &
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nvt_h0 allocation failure, node: ', idnode
     Call error(0)
  End If
...
! propagate chit set
  ...
  chit1 = chit + tstep*(2.0_wp*engke-ceng)/qmass
...
! update kinetic energy and stress at full step
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nvt_h0 deallocation failure, node: ', idnode
     Call error(0)
  End If

End Subroutine nvt_h0_lfv
