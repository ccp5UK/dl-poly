...
\item Extended EAM potential ({\bf eeam})\index{potential!EEAM}
...
\item Extended EAM density correction \\
No long-ranged corrections apply beyond $r_{\rm met}$.
...
No long-ranged corrections apply beyond $r_{\rm met}$.
\item Extended EAM energy correction \\
No long-ranged corrections apply beyond $r_{\rm met}$.
...
No long-ranged corrections apply beyond $r_{\rm met}$.
\item Extended EAM virial correction \\
No long-ranged corrections apply beyond $r_{\rm met}$.
...
It is worth noting that in the EEAM potential framework all densities
can be different \cite{hepburn-08a,lau-07a}:
\begin{equation}
\rho^{AA}_{ij}(r_{ij}) \neq \rho^{BB}_{ij}(r_{ij}) \neq \rho^{AB}_{ij}(r_{ij}) \neq
\rho^{BA}_{ij}(r_{ij})~~!
\end{equation}

Thus when specifying these potentials in the \D FIELD file for an
...
functions both the EAM and EEAM potentias require further specification
of $n$ functional forms of the density dependence (i.e. the embedding
function $F(\rho_{i})$ in (\ref{um})).
