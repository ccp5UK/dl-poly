...
formula below.  The distinction between the error function $\rm erf$
and the more usual complementary error function $\rm erfc$ found in
...
{r_{nj}} {\rm erfc}(\alpha r_{nj}) - \phantom{xxxx} \nonumber \\
...
\begin{equation}
\alpha = q_{s}^{2}/k_{2}~~, \label{druder}
\end{equation}
...
Thole dumping factor is (optionally) provided with the above directive
in CONTROL, then a default one of 1.3 is assumed for all inducible particles.
...
recovered.  {\bf Note} that in \D if $q_{s}$ is recovered, it has an
opposite sign to that of $q_{c}$!  In the special case when all Druders'
force constants, $k_{2}$, are undefined or zero (in FIELD), \D will resort
to using the same one of 1000~kcal~mol$^{-1}$\AA$^{-2}$ for all core-shell
units, following the CHARMM recommendations in \cite{mackerell-16a}.
