...
\item Morse modified \cite{pedone2006}: ({\bf mstw})
  \begin{equation}
    U(r_{ij}) = E_{o}~[\{1-\exp(-k(r_{ij}-r_{o}))\}^{2}-1]+\frac{c_{ij}}{r^{12}}
  \end{equation}
\item Rydberg: ({\bf ryd})
  \begin{equation}
    U(r_{ij}) = (a+br)\exp(-r/\rho)
  \end{equation}
  ...
  \begin{equation}
    \begin{split}
      U(r_{ij}) =& \frac{Z_1Z_2e^2}{4\pi\varepsilon_0r}\sum_{i=1}^{4}b_i e^{-c_ir/a} \\
               ...
               a_B =& 0.52917721067\AA
    \end{split}
  \end{equation}
\item ZBL mixed with Morse, \cite{trachenko2003}: ({\bf zbls})
  \begin{equation}
      U(r_{ij}) = f(r_{ij})U_{ZBL}(r_{ij})+(1-f(r))U_{morse}(r_{ij})
  \end{equation}
  \begin{equation*}
     f(r)=\left\{
       \begin{array}{ll}
         1-e^{-(r_m-r)/\xi}/2 & r<r_m \\
         e^{-(r-r_m)/\xi}/2 & r\geq r_m\\
       \end{array}
       \right.
  \end{equation*}
