...
\>                                              \> \\
\> {\bf print ana}lysis                         \> print analysed PDFs in OUTPUT and corresponding files *DAT PMF*. \\
\>                                              \> \\
...
\>                                              \> \phantom{x} ~~~~~~12-6 or LJ or DPD or AMOE or WCA type. \\
...
from comparison with a default simualtion and comparisons are carried over
a few hundreds of timesteps.  This is usually accounting for over $90\%$ of
the time tosolution.} of the order of $10 \div 30\%$ for force-fields
...
according to the user specifications in the FILED files.
...
$U(r) = \infty~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~:~r_{ij} \ge R_{o} + \Delta$ \\
            &                    &            &          &          &       & \\
{\bf amoe}  & AMOEBA \cite{ponder-10a} & $k$  & $r_{o}$  &          &       &
$U(r) = k~\delta^{2}\left[1-2.55~\delta+(7/12)~2.55~\delta^{2}\right]~;~\delta~ = r-r_{o}$ \\
{\bf -amo}  & FF bond            &            &          &          &       & \\
            &                    &            &          &          &       & \\
\hline
\end{tabular}
...
$ [B~(r_{ij} - r_{ij}^{o})+C~(r_{ik} - r_{ik}^{o})]$ \\
           &                     &     &              &            &            & \\
{\bf amoe} & AMOEBA \cite{ponder-10a} & $k$ & $\theta_{0}$ &       &            &
...
$U(r) = 4 \epsilon \left[ \left( \frac{\sigma}{r} \right)^{12} - \left( \frac{\sigma}{r}\right)^{6} \right]$ \\
           &                           &            &          &          &         &                      & \\
{\bf nm}   & n-m \cite{clarke-86a}     & $E_{o}$    & $n$      & $m$      & $r_{0}$ &                      &
...
$U(r) = 0~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~: r_{ij} \ge 2^{1 \over 6}~\sigma + \Delta$ \\
           &                           &            &          &          &         &                      & \\
{\bf dpd}  & Standard DPD \cite{groot-71a} & $A$    & $r_{c}$$^{\ddagger}$ &        &         &                      &
$U(r) = \frac{A}{2}~r_{c}~\left(1-\frac{r}{r_{c}}\right)^{2}~:~r < r_{c}$ \\
           & (Groot-Warren)             &           &          &          &         &                      &
$U(r) = 0~~~~~~~~~~~~~~~~~~~:~r \ge r_{c}$ \\
           &                           &            &          &          &         &                      & \\
{\bf amoe} & AMOEBA \cite{ponder-10a} & $\epsilon$ & $r_{o}$   &          &         &                      &
...
\subsection{The TAB* Files for Intramolecular Interactions}
\label{tab-files}
In order to set up a (possibly coarse-grained) molecular system 
with user-defined tabulated potential and force data, the user 
should refer to Sections~\ref{cg-intro} and~\ref{bonded-tables}. 
Below only the TAB* files for {\em non-bonded} interations are 
described. 
...
\> {\tt potential} \> real \> potential at the abscissa grid point in {\bf units} as specified in FILED \\
