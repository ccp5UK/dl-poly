...
! - leapfrog verlet with Hoover thermostat
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Real( Kind = wp ), Intent( In    ) :: mndis,mxdis,sigma,taut
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nvt_h1 allocation failure, node: ', idnode
     Call error(0)
  End If
...
! propagate chit set
  ...
  chit1 = chit + tstep*(2.0_wp*(engke+engrot)-ceng)/qmass
...
! update kinetic energy and stress at full step
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nvt_h1 deallocation failure, node: ', idnode
     Call error(0)
  End If

End Subroutine nvt_h1_lfv
