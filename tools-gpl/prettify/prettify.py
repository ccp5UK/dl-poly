...
def upcaseStringKeywords(line):
    ...
    while start < len(line):
        ...
        res = res + TO_UPCASE_RE.sub(lambda match: match.group("toUpcase").capitalize(),
                                     m.group("commands"))
        if m.group("comment"):
...
def prettifyFile(infile, filename, normalize_use, decl_linelength, 
...
def main(argv=None):
    ...
    defaultsDict = {'upcase': 1, 'normalize-use': 1, 'omp-upcase': 1,
                    'decl-linelength': 100, 'decl-offset-use': 32,'decl-offset-var': 22,
                    'reformat': 1, 'indent': 2, 'whitespace': 3,
                    'replace': 0,
                    'stdout': 0,
                    'do-backup': 0,
    ...
    --normalize-use
             ...
             is controlled by --decl-offset-use=n or --decl-offset-var=n.
    --reformat
    ...
    for arg in argv[1:]:
        ...
        if m:
            ...
            m = re.match(
                r"--(indent|whitespace|decl-linelength)=(.*)", arg)
            if m:
