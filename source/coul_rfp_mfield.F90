Subroutine coul_rfp_mfield &
           (iatm,rcut,alpha,epsq,xxt,yyt,zzt,rrt)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! dl_poly_4 subroutine for calculating the permanent electrostatic field
! for a system of multipoles assuming a reaction field potential
! (corrected for the existence of a dipole moment outside rcut)
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Integer           :: fail,idi,ai,jatm,aj,s1,s2,s3,m
  Integer           :: s11,s21,s31,ii,jj,k,n

  Real( Kind = wp ) :: scl,rrr,fix,fiy,fiz,fx,fy,fz,        &
                       tix,tiy,tiz,tjx,tjy,tjz,vk0,vk1,vk2, &
                       t1,t2,t3,tj,ti,sx,sy,sz,tmp,ppp,erfcr
...
! compute derivatives of 'r^2'
              ...
              Call coul_deriv(-2,mxompl+1,xxt(m),yyt(m),zzt(m),rrr,a1)
...
End Subroutine coul_rfp_mfield
