...
! split anisotropic from semi-isotropic barostats (iso=0,1,2,3)
        ...
        chit1 = chit + tstep*(2.0_wp*(engke+engrot)+pmass*chip0**2-ceng)/qmass
        If (iso == 0) Then
           eta1=(eta + tstep*(strcom+strcon+strpmf+stress+strkin - (press*uni+strext)*volm)/pmass)*Exp(-tstep*chit2)
        Else
           If      (iso == 2) Then
              eta1(1)=(eta(1) + tstep*( strcom(1)+strcon(1)+strpmf(1)+stress(1)+strkin(1) - &
                                        (press+strext(1)-ten/h_z)*volm )/pmass)*Exp(-tstep*chit2)
              eta1(5)=(eta(5) + tstep*( strcom(5)+strcon(5)+strpmf(5)+stress(5)+strkin(5) - &
                                        (press+strext(5)-ten/h_z)*volm )/pmass)*Exp(-tstep*chit2)
           Else If (iso == 3) Then
              eta1(1)=(0.5_wp*(eta(1)+eta(5)) + tstep*( 0.5_wp*              &
              ...
              eta1(5)=eta1(1)
           End If
           eta1(9)=(eta(9) + tstep*( strcom(9)+strcon(9)+strpmf(9)+stress(9)+strkin(9) - &
