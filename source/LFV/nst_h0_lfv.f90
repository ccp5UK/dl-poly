...
! molecular dynamics - leapfrog verlet with Hoover thermostat and
! barostat (anisotropic pressure control)
!
...
! split anisotropic from semi-isotropic barostats (iso=0,1,2,3)
  ...
  If (iso == 0) Then
     eta1=(eta + tstep*(stress+strkin - (press*uni+strext)*volm)/pmass)*Exp(-tstep*chit2)
  Else
     If      (iso == 2) Then
        eta1(1)=(eta(1) + tstep*( stress(1)+strkin(1) - &
                                  (press+strext(1)-ten/h_z)*volm )/pmass)*Exp(-tstep*chit2)
        eta1(5)=(eta(5) + tstep*( stress(5)+strkin(5) - &
     ...
     Else If (iso == 3) Then
        ...
        eta1(5)=eta1(1)
     End If
     eta1(9)=(eta(9) + tstep*( stress(9)+strkin(9) - (press+strext(9))*volm )/pmass)*Exp(-tstep*chit2)
  End If
