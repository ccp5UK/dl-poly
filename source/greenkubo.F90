Module greenkubo
  ...
  Type, Public :: greenkubo_type
    ...
    Real( Kind = wp ), Allocatable, Public :: vafdata(:,:),vaftime(:),vaf(:,:)
...
Contains
  ...
  Subroutine cleanup(green)
    ...
    If (Allocated(green%vafstep)) Then
      Deallocate(green%vafstep)
    End If
    ...
    End If
    If (Allocated(green%vaftime)) Then
      Deallocate(green%vaftime)
    End If
