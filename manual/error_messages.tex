...
\subsubsection*{\underline{Message 172}: error - duplicate intra-molecular entries specified \\
in TABBND\textbar\textbar TABANG\textbar\textbar TABDIH\textbar\textbar TABINV}
...
defined.  Make sure that subroutine \\
{\sc three\_body\_forces}
...
\subsubsection*{\underline{Message 660}: error - bond-length $>$ cutoff in TABBND or cutoff for PDF collection}

A bond has reached a length beyond the cutoff over which (i) its interactions
are defined in TABBND or (ii) its potential distribution function is sampled.

\noindent \underline{\em Action}:

If there is a TABBND present, reconstruct TABBND with the original interaction
potentials defined over a larger cutoff and try to run the system with new TABBND.
If bonds PDFs are collected then increase the PDF bond cutoff value in CONTROL
and try to run the system.

\subsubsection*{\underline{Message 670}: error - insufficient electronic temperature cells for TTM heat diffusion}

The number of coarse-grained electronic temperature (CET) cells for the
heat diffusion calculations of the two-temperature model (TTM) in any direction
(x, y or z) is less than the number of coarse-grained ionic temperature (CIT)
cells.

\noindent \underline{\em Action}:

Examine the OUTPUT for the number of ionic temperature cells and modify the
{\bf ttm ncet} directive in the CONTROL file to ensure there are at least as many
electronic temperature cells.

\subsubsection*{\underline{Message 680}: error - rpad too large for calculation of ionic temperatures}
...
Reduce the padding distance for pair interactions with the {\bf rpad} directive in
the CONTROL file.

\subsubsection*{\underline{Message 681}: error - electronic specific heat not fully specified}

Not enough information is given for electronic specific heat capacity functions
(other than a constant value) used in two-temperature model calculations.

\noindent \underline{\em Action}:

Ensure that two positive parameters are given with either the {\bf ttm cetanh} or
{\bf ttm celin} directives in the CONTROL file.

\subsubsection*{\underline{Message 682}: error - thermal conductivity of metal not specified}

No information given for thermal conductivity of metal used in two-temperature
model calculations.

\noindent \underline{\em Action}:

Ensure that a positive parameter is given with either the {\bf ttm keconst} or
{\bf ttm kedrude} directives in the CONTROL file.

\subsubsection*{\underline{Message 683}: error - thermal diffusivity of non-metal not specified}

No information given for thermal diffusivity of non-metal used in two-temperature
model calculations.

\noindent \underline{\em Action}:

Ensure that a positive parameter is given with the {\bf ttm diff} directive in the
CONTROL file.

\subsubsection*{\underline{Message 684}: error - cannot find or open thermal conductivity table file (Ke.dat)}

No readable file (Ke.dat) is available to read tabulated thermal conductivities.

\noindent \underline{\em Action}:

Ensure that a readable text file named Ke.dat is available in the directory
where \D is being run. (This must be supplied if the {\bf ttm ketab} directive
is given in the CONTROL file.)

\subsubsection*{\underline{Message 685}: error - no data found in thermal conductivity table file (Ke.dat)}

No tabulated thermal conductivities can be read from the Ke.dat file.

\noindent \underline{\em Action}:

Ensure that the Ke.dat file is formatted correctly in two columns: temperature
and thermal conductivity. Temperatures should be greater than or equal to 0 kelvin.

\subsubsection*{\underline{Message 686}: error - cannot find or open volumetric heat capacity table file (Ce.dat)}

No readable file (Ce.dat) is available to read tabulated volumetric heat capacities.

\noindent \underline{\em Action}:

Ensure that a readable text file named Ce.dat is available in the directory
where \D is being run. (This must be supplied if the {\bf ttm cetab} directive
is given in the CONTROL file.)

\subsubsection*{\underline{Message 687}: error - no data found in volumetric heat capacity table file (Ce.dat)}

No tabulated volumetric heat capacities can be read from the Ce.dat file.

\noindent \underline{\em Action}:

Ensure that the Ce.dat file is formatted correctly in two columns: temperature
and volumetric heat capacity (equal to product of specific heat capacity and
density). Temperatures should be greater than or equal to 0 kelvin.

\subsubsection*{\underline{Message 688}: error - cannot find or open thermal diffusivity table file (De.dat)}

No readable file (Ce.dat) is available to read tabulated volumetric heat capacities.

\noindent \underline{\em Action}:

Ensure that a readable text file named De.dat is available in the directory
where \D is being run. (This must be supplied if the {\bf ttm detab} directive
is given in the CONTROL file.)

\subsubsection*{\underline{Message 689}: error - no data found in thermal diffusivity table file (De.dat)}

No tabulated thermal diffusivities can be read from the De.dat file.

\noindent \underline{\em Action}:

Ensure that the De.dat file is formatted correctly in two columns: temperature
and thermal diffusivity. Temperatures should be greater than or equal to 0 kelvin.

\subsubsection*{\underline{Message 690}: error - cannot find or open coupling constant table file (g.dat)}

No readable file (g.dat) is available to read tabulated electron-phonon coupling constants.

\noindent \underline{\em Action}:

Ensure that a readable text file named g.dat is available in the directory
where \D is being run. (This must be supplied if the {\bf ttm gvar} directive
is given in the CONTROL file.)

\subsubsection*{\underline{Message 691}: error - no data found in coupling constant table file (g.dat)}

No tabulated thermal conductivities can be read from the g.dat file.

\noindent \underline{\em Action}:

Ensure that the g.dat file is formatted correctly in two columns: temperature
and electron-phonon coupling constant. Temperatures should be greater than
or equal to 0 kelvin.

\subsubsection*{\underline{Message 692}: error - end of file encountered in table file (Ke.dat, Ce.dat, De.dat or g.dat)}

\D encountered the end of one of the tabulated files (Ke.dat, Ce.dat, De.dat or g.dat)
for two-temperature model calculations prematurely.

\noindent \underline{\em Action}:

Check that the tabulated files are not corrupted or incomplete in some way.

\subsubsection*{\underline{Message 693}: error - negative electronic temperature: instability in electronic heat diffusion equation}

A negative (non-physical) electronic temperature has been obtained during
solution of the thermal diffusion equation used in the two-temperature model.
This is an indication of instability in the numerical solution of this partial
differential equation.

\noindent \underline{\em Action}:

This error should not happen due to careful selection of the timestep used
for the explicit difference solver. In some limited circumstances, it may be
possible to improve the solver stability by decreasing the value of
{\tt fopttstep} in {\sc ttm\_thermal\_diffusion}, which is used to scale the
timestep.

\subsubsection*{\underline{Message 694}: error - electronic temperature restart file (DUMP\_E) does not exist}

The DUMP\_E file (containing electronic temperatures for restarting
simulations with the two-temperature model) does not exist.

\noindent \underline{\em Action}:

A DUMP\_E file should be supplied if the {\bf restart} directive is included in
the CONTROL file.

\subsubsection*{\underline{Message 695}: error - mismatch in electronic temperature lattice sizes between restart (DUMP\_E) and CONTROL files}

The electronic temperature lattice size (number of CET cells) given in
the DUMP\_E restart file for two-temperature model simulations does
not correspond to the size given in the CONTROL file.

\noindent \underline{\em Action}:

Ensure that the {\bf ttm ncet} directive in the CONTROL file matches up with
the three numbers in the first line of the DUMP\_E file.

\subsubsection*{\underline{Message 696}: error - cannot read electronic temperature restart (DUMP\_E) file}
...
Check that the DUMP\_E file is not corrupted or incomplete in some way.
...
\subsubsection*{\underline{Message 1083}: error - allocation failure in ttm\_module $->$ allocate\_ttm\_arrays}
...
\subsubsection*{\underline{Message 1084}: error - deallocation failure in ttm\_module $->$ deallocate\_ttm\_arrays}
...
\subsubsection*{\underline{Message 1085}: error - allocation failure in ttm\_ion\_temperature}
...
\subsubsection*{\underline{Message 1086}: error - deallocation failure in ttm\_ion\_temperature}
...
\subsubsection*{\underline{Message 1087}: error - allocation failure in ttm\_thermal\_diffusion}
...
\subsubsection*{\underline{Message 1088}: error - deallocation failure in ttm\_thermal\_diffusion}
...
\subsubsection*{\underline{Message 1089}: error - allocation failure in ttm\_track\_module $->$ depoinit}
...
\subsubsection*{\underline{Message 1090}: error - deallocation failure in ttm\_track\_module $->$ depoevolve}
