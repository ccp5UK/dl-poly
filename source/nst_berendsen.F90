Module nst_berendsen
  ...
  Public :: nst_b0_vv, nst_b1_vv

Contains

  Subroutine nst_b0_vv(stage, lvar, mndis, mxdis, mxstp, tstep, &
                       stress, &
                       strkin, engke, &
...
! uni is the diagonal unit matrix
    ...
    If (Any(fail > 0)) Then
      Write (message, '(a)') 'nst_b0 allocation failure'
    ...
    If (stage == VV_FIRST_STAGE) Then
      ...
      ! iterate forces, stat%strcon and thermo%eta

      Do iter = 1, thermo%mxiter

        ! Berendsen barostat and thermostat are not coupled
        ! calculate Berendsen barostat: thermo%eta, iterate stat%strcon and stat%strpmf
        ...
        Do i = 1, config%natms
          If (config%weight(i) > 1.0e-6_wp) Then
            ...
            config%parts(i)%xxx = tstep * config%vxx(i) + xxt(i) * thermo%eta(1) + yyt(i) * thermo%eta(2) + zzt(i) * thermo%eta(3)
            config%parts(i)%yyy = tstep * config%vyy(i) + xxt(i) * thermo%eta(2) + yyt(i) * thermo%eta(5) + zzt(i) * thermo%eta(6)
            config%parts(i)%zzz = tstep * config%vzz(i) + xxt(i) * thermo%eta(3) + yyt(i) * thermo%eta(6) + zzt(i) * thermo%eta(9)
          End If
        End Do
        ! update config%cell parameters: anisotropic

        Call mat_mul(thermo%eta, czero, config%cell)
      ...
      If (cons%megcon == 0 .and. pmf%megpmf == 0) Call mat_mul(thermo%eta, czero, config%cell)

      ! update volume and construct a 'mock' thermo%chi_p

      Call dcell(config%cell, celprp)
      thermo%chi_p = celprp(10) / config%volm
    ...
    If (Any(fail > 0)) Then
      Write (message, '(a)') 'nst_b0 deallocation failure'
      Call error(0, message)
    End If

  End Subroutine nst_b0_vv

  Subroutine nst_b1_vv(stage, lvar, mndis, mxdis, mxstp, tstep, &
                       stress, &
    ...
    ! - velocity verlet with Berendsen thermostat and
    ! anisotropic pressure control (not symplectic)
    !
    ! isothermal compressibility (beta) set to that of liquid water
    ! = 0.007372 dl_poly units
    ...
    Integer                    :: fail(1:15), i, i1, i2, irgd, iter, j, jrgd, krgd, lrgd, matms, &
                                  rgdtyp
    Logical, Allocatable       :: lstitr(:)
    Real(Kind=wp)              :: celprp(1:10), czero(1:9), fmx, fmy, fmz, hstep, mxdr, opx, opy, &
                                  opz, qt0, qt1, qt2, qt3, rot(1:9), rstep, tmp, tqx, tqy, tqz, &
                                  trx, try, trz, vom(1:3), vpx, vpy, vpz, x(1:1), y(1:1), z(1:1)
...
! uni is the diagonal unit matrix
    ...
    If (Any(fail > 0)) Then
      Write (message, '(a)') 'nst_b1 allocation failure'
      Call error(0, message)
    End If
      ...
      Do irgd = 1, rigid%n_types
        ...
        If (rigid%frozen(0, rgdtyp) < lrgd) Then
          ...
          rigid%xxx(irgd) = tstep * rigid%vxx(irgd) + &
                            rgdxxt(irgd) * thermo%eta(1) + rgdyyt(irgd) * thermo%eta(2) + rgdzzt(irgd) * thermo%eta(3)
          rigid%yyy(irgd) = tstep * rigid%vyy(irgd) + &
                            rgdxxt(irgd) * thermo%eta(2) + rgdyyt(irgd) * thermo%eta(5) + rgdzzt(irgd) * thermo%eta(6)
          rigid%zzz(irgd) = tstep * rigid%vzz(irgd) + &
                            rgdxxt(irgd) * thermo%eta(3) + rgdyyt(irgd) * thermo%eta(6) + rgdzzt(irgd) * thermo%eta(9)
            ...
            If (i <= config%natms) Then
              If (rigid%frozen(jrgd, rgdtyp) == 0) Then
                ...
                If (thermo%unsafe) Then
                  config%vxx(i) = xxt(i) * thermo%eta(1) + yyt(i) * thermo%eta(2) + zzt(i) * thermo%eta(3)
                  config%vyy(i) = xxt(i) * thermo%eta(2) + yyt(i) * thermo%eta(5) + zzt(i) * thermo%eta(6)
                  config%vzz(i) = xxt(i) * thermo%eta(3) + yyt(i) * thermo%eta(6) + zzt(i) * thermo%eta(9)
        ...
        Else
          ...
          rigid%xxx(irgd) = rgdxxt(irgd) * thermo%eta(1) + rgdyyt(irgd) * thermo%eta(2) + rgdzzt(irgd) * thermo%eta(3)
          rigid%yyy(irgd) = rgdxxt(irgd) * thermo%eta(2) + rgdyyt(irgd) * thermo%eta(5) + rgdzzt(irgd) * thermo%eta(6)
          rigid%zzz(irgd) = rgdxxt(irgd) * thermo%eta(3) + rgdyyt(irgd) * thermo%eta(6) + rgdzzt(irgd) * thermo%eta(9)
    ...
    If (Any(fail > 0)) Then
      Write (message, '(a)') 'nst_b1 deallocation failure'
      Call error(0, message)
    End If

  End Subroutine nst_b1_vv
End Module nst_berendsen
