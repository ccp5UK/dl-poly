...
Contains
  ...
  Subroutine read_control( lfce,impa,ttm,dfcts,rigid, &
    ...
    Write(banner(6),'(a)') ''
    If(flow%newjob_read_control) Call info(banner,6,.true.)

    If(flow%newjob_read_control) Call info('simulation control parameters',.true.)
    ...
    Do
      ...
      Else If (word(1:5) == 'l_eng') Then
        devel%l_eng = .true.
        If(flow%newjob_read_control) Call info('%%% OUTPUT contains an extra last line with E_tot !!! %%%',.true.)
      Else If (word(1:6) == 'l_rout') Then
        devel%l_rout = .true.
        If(flow%newjob_read_control) Call info('%%% REVIVE writing in ASCII opted !!! %%%',.true.)
      Else If (word(1:5) == 'l_rin') Then
        devel%l_rin = .true.
        If(flow%newjob_read_control) Call info('%%% REVOLD reading in ASCII opted !!! %%%',.true.)
      Else If (word(1:5) == 'l_org') Then
        devel%l_org = .true.
        devel%l_trm  = .true.

        If(flow%newjob_read_control)Then
          Call info('%%% translate CONFIG along a vector into CFGORG after reading input & terminate !!! %%%',.true.)
      End If  
        If(flow%newjob_read_control) Call info('%%% vector and config level read as follows: %%%',.true.)
        ...
        If(flow%newjob_read_control) Call info(messages,3,.true.)
      ...
      Else If (word(1:5) == 'l_scl') Then
        ...
        If(flow%newjob_read_control) Call info(messages,7,.true.)

        If (tmp > zero_plus) Then
          devel%l_scl = .true.
          devel%l_trm  = .true.
        Else
          If(flow%newjob_read_control) Call info('%%% OPTION ABORTED DUE TO ZERO VOLUME !!! %%%',.true.)
          devel%l_trm  = .true.
        End If
      Else If (word(1:5) == 'l_his') Then
        devel%l_his = .true.
        devel%l_trm = .true.
        If(flow%newjob_read_control) Call info('%%% generate HISTORY after reading input & terminate !!! %%%',.true.)
      Else If (word(1:5) == 'l_tim') Then
        !        l_tim = .true.  ! done in scan_development
        If(flow%newjob_read_control) Call info('%%% generate detailed timing !!! %%%',.true.)
      Else If (word(1:5) == 'l_tor') Then
        devel%l_tor = .true.
        If(flow%newjob_read_control) Call info('%%% Turn off production of REVCON & REVIVE !!! %%%',.true.)
      Else If (word(1:5) == 'l_trm') Then
        devel%l_trm = .true.
        If(flow%newjob_read_control) Call info('%%% Terminate gracefully before initialisation !!! %%%',.true.)
      Else If (word(1:5) == 'l_dis') Then
        ...
        If(flow%newjob_read_control) Call info(message,.true.)
      ...
      Else If (word(1:3) == 'vdw') Then
        ...
        If      (word1(1:6) == 'direct') Then
          ...
          vdws%l_direct = .true.
          If(flow%newjob_read_control) Call info('vdw direct option on',.true.)

        Else If (word1(1:3) == 'mix') Then

          ! mixing type keywords

          If(flow%newjob_read_control) Call info('vdw cross terms mixing opted (for undefined mixed potentials)',.true.)
          If(flow%newjob_read_control) Call info('mixing is limited to potentials of the same type only',.true.)
          If(flow%newjob_read_control) Call info('mixing restricted to LJ-like potentials (12-6,LJ,WCA,DPD,AMOEBA)',.true.)
          ...
          Else If (word2(1:4) == 'hoge') Then
            ...
            If(flow%newjob_read_control) Call info('type of mixing selected - Hogervorst (good hope) :: ' &
              //'e_ij=(e_i*e_j)^(1/2) ; s_ij=(s_i*s_j)^(1/2)',.true.)
          Else If (word2(1:4) == 'halg') Then
            vdws%mixing = MIX_HALGREN
            If(flow%newjob_read_control) Call info('type of mixing selected - Halgren HHG :: ' &
          ...
          Else If (word2(1:4) == 'wald') Then
            ...
            If(flow%newjob_read_control) Call info('type of mixing selected - Waldman–Hagler :: ' &
          ...
          Else If (word2(1:4) == 'tang') Then
            ...
            If(flow%newjob_read_control) Call info('type of mixing selected - Tang-Toennies :: ' &
              //' e_ij=[(e_i*s_i^6)*(e_j*s_j^6)] / {[(e_i*s_i^12)^(1/13)+(e_j*s_j^12)^(1/13)]/2}^13',.true.)
            If(flow%newjob_read_control) Call info(Repeat(' ',43)//'s_ij={[(e_i*s_i^6)*(e_j*s_j^6)]^(1/2) / e_ij}^(1/6)',.true.)
          Else If (word2(1:4) == 'func') Then
            vdws%mixing = MIX_FUNCTIONAL
            If(flow%newjob_read_control) Call info('type of mixing selected - Functional :: ' &
        ...
        Else If (word1(1:5) == 'shift') Then
          ...
          If(flow%newjob_read_control) Call info('vdw force-shifting option on',.true.)
        Else
          Call strip_blanks(record)
      ...
      Else If (word(1:5) == 'metal') Then
        ...
        If      (word(1:6) == 'direct') Then
          ...
          If(flow%newjob_read_control) Call info('metal direct option on',.true.)
          If (met%tab > 0) Then
        ...
        Else If (word(1:7) == 'sqrtrho') Then
          ...
          If(flow%newjob_read_control) Call info('metal sqrtrho option on',.true.)
          If (met%tab > 0) Then
            met%l_emb = .true.
          Else
      ...
      Else If (word(1:4) == 'slab') Then
        ...
        If(flow%newjob_read_control) Call info('slab option on',.true.)
      ...
      Else If (word(1:6) == 'impact') Then
        ...
        If(flow%newjob_read_control) Call info(messages,5,.true.)
      ...
      Else If (word(1:4) == 'zero') Then
        ...
        If(flow%newjob_read_control) Call info('zero K optimisation on (during equilibration)',.true.)
        ...
        Else
          ltemp  = .true.
          thermo%temp = 10.0_wp
          If(flow%newjob_read_control) Call info('fire option off - target temperature reset to 10 Kelvin',.true.)
        End If
      ...
      Else If (word(1:4) == 'pres') Then
        ...
        If (word(1:6) == 'tensor') Then
          ...
          If(flow%newjob_read_control) Call info('simulation pressure tensor (katms):',.true.)
      ...
      Else If (word(1:7) == 'restart') Then
        ...
        If (word(1:7) == 'noscale' .or. word(1:7) == 'unscale') Then
          ...
          If(flow%newjob_read_control) Call info('unscaled restart requested (starting a new simulation)',.true.)
        Else If (word(1:5) == 'scale') Then
          flow%restart_key = RESTART_KEY_SCALE
          If(flow%newjob_read_control) Call info('scaled restart requested (starting a new simulation)',.true.)
        Else
          flow%restart_key = RESTART_KEY_OLD
          If(flow%newjob_read_control) Call info('restart requested (continuing an old simulation)',.true.)
      ...
      Else If (word(1:7) == 'collect') Then
        ...
        If(flow%newjob_read_control) Call info('equilibration included in overall averages',.true.)
      ...
      Else If (word(1:6) == 'pseudo') Then
        ...
        If (config%width/4.0_wp > thermo%width_pseudo) Then
          ...
          If (comm%idnode == 0) Then
            If(flow%newjob_read_control) Call info('pseudo thermostat attached to MD cell boundary',.true.)
            If      (thermo%key_pseudo == 0) Then
              If(flow%newjob_read_control) Call info('thermostat control: Langevin + direct temperature scaling',.true.)
            Else If (thermo%key_pseudo == 1) Then
              If(flow%newjob_read_control) Call info('thermostat control: Langevin temperature scaling',.true.)
            Else If (thermo%key_pseudo == 2) Then
              If(flow%newjob_read_control) Call info('thermostat control: gaussian temperature scaling',.true.)
            Else If (thermo%key_pseudo == 3) Then
              If(flow%newjob_read_control) Call info('thermostat control: direct temperature scaling',.true.)
            End If
          ...
          Else
            If(flow%newjob_read_control) Call info('thermostat thickness insufficient - reset to 2 Angs',.true.)
          End If
        Else
      ...
      Else If (word(1:5) == 'minim' .or. word(1:5) == 'optim') Then
        ...
        If (word2(1:5) == 'minim') Then
          ...
          If(flow%newjob_read_control) Call info(messages,4,.true.)
      ...
      Else If (word(1:5) == 'scale') Then
        ...
        If(flow%newjob_read_control) Call info('temperature scaling on (during equilibration)',.true.)
      ...
      Else If (word(1:5) == 'polar') Then
        ...
        If (word(1:6) == 'charmm') Then
          ...
          Else
            ...
            If(flow%newjob_read_control) Call info('Extended Coulombic eXclusion activated for CHARMM polarisation',.true.)
          End If
        End If
      ...
      Else If (word(1:8) == 'ensemble') Then
        ...
        If      (word(1:3) == 'nve' .or. word(1:3) == 'pmf') Then
          ...
          If(flow%newjob_read_control) Call info('Ensemble : NVE (Microcanonical)',.true.)
        ...
        Else If (word(1:3) == 'nvt') Then
          ...
          If      (word(1:5) == 'evans') Then
            ...
            If(flow%newjob_read_control) Call info('Ensemble : NVT Evans (Isokinetic)',.true.)
            If(flow%newjob_read_control) Call info('Gaussian temperature constraints in use',.true.)
          ...
          Else If (word(1:4) == 'lang') Then
            ...
            If(flow%newjob_read_control) Call info('Ensemble : NVT Langevin (Stochastic Dynamics)',.true.)
          ...
          Else If (word(1:3) == 'ber') Then
            ...
            If(flow%newjob_read_control) Call info('Ensemble : NVT Berendsen',.true.)
          ...
          Else If (word(1:6) == 'hoover') Then
            ...
            If(flow%newjob_read_control) Call info('Ensemble : NVT Nose-Hoover',.true.)
            Write(message,'(a,1p,e12.4)') 'thermostat relaxation time (ps) ',thermo%tau_t
            If(flow%newjob_read_control) Call info(message,.true.)
          ...
          Else If (word(1:3) == 'dpd') Then
            ...
            If(flow%newjob_read_control) Call info('Ensemble : NVT dpd (Dissipative Particle Dynamics)',.true.)
            ...
            If      (thermo%key_dpd == DPD_FIRST_ORDER) Then
              ...
              If(flow%newjob_read_control) Call info("Ensemble type : Shardlow's first order splitting (S1)",.true.)
            Else If (thermo%key_dpd == DPD_SECOND_ORDER) Then
              ...
              If(flow%newjob_read_control) Call info("Ensemble type : Shardlow's second order splitting (S2)",.true.)
            Else
              Call strip_blanks(record)
        ...
        Else If (word(1:3) == 'nst') Then
          ...
          If (word(1:4) == 'lang') Then
            ...
            If      (word(1:4) == 'area') Then
              ...
              If(flow%newjob_read_control) Call info('semi-isotropic barostat : constant normal pressure (Pn) &',.true.)
              If(flow%newjob_read_control) Call info('       (N-Pn-A-T)       : constant surface area (A)',.true.)
            Else If (word(1:4) == 'tens') Then
              ...
              If (word(1:4) == 'semi') Then
                ...
                If(flow%newjob_read_control) Call info('semi-isotropic barostat : semi-orthorhombic MD cell constraints',.true.)
            ...
            Else If (word(1:4) == 'orth') Then
              ...
              If (Len_Trim(word) == 0) Then
                thermo%iso = CONSTRAINT_SURFACE_TENSION
                If(flow%newjob_read_control) Call info('semi-isotropic barostat : orthorhombic MD cell constraints',.true.)
      ...
      Else If (word(1:5) == 'ewald' .or. word(1:4) == 'spme') Then
        ...
        Else
          ...
          If(flow%newjob_read_control) Call info('Electrostatics : Smooth Particle Mesh Ewald',.true.)
      ...
      Else If (word(1:6) == 'distan') Then
        ...
        If(flow%newjob_read_control) Call info('Electrostatics : Distance Dependent Dielectric',.true.)
      ...
      Else If (word(1:4) == 'coul') Then
        ...
        If(flow%newjob_read_control) Call info('Electrostatics : Coulombic Potential',.true.)
      ...
      Else If (word(1:5) == 'shift') Then
        ...
        If(flow%newjob_read_control) Call info('Electrostatics : Force-Shifted Coulombic Potential',.true.)
        ...
        If (electro%alpha > zero_plus) Then
          If(flow%newjob_read_control) Call info('Fennell damping applied',.true.)
      ...
      Else If (word(1:8) == 'reaction') Then
        ...
        If(flow%newjob_read_control) Call info('Electrostatics : Reaction Field',.true.)
      ...
      Else If (word(1:5) == 'poiss' .or. word(1:5) == 'psolv' ) Then
        ...
        If(flow%newjob_read_control) Call info('Electrostatics : Poisson equation solver',.true.)
      ...
      Else If (word(1:5) == 'exclu') Then
        ...
        electro%lecx = .true.
        If(flow%newjob_read_control) Call info('Extended Coulombic eXclusion opted for',.true.)
      ...
      Else If (word(1:3) == 'cap') Then
        ...
        If(flow%newjob_read_control) Call info(messages,2,.true.)
      ...
      Else If (word(1:2) == 'no') Then
        ...
        Else If (word1(1:3) == 'str' ) Then
          ...
          If(flow%newjob_read_control) Call info('no strict option on',.true.)
          ...
          If(flow%newjob_read_control) Call info(banner,9,.true.)

        Else If (word1(1:3) == 'top' ) Then

          If(flow%newjob_read_control) Call info('no topology option on (avoids printing extended FIELD topology in OUTPUT)',.true.)
        ...
        Else If (word1(1:3) == 'vom' ) Then ! "no vom" should be used with TTM
          ...
          If (.not. ttm%l_ttm) Then
            ...
            End If  
            If(flow%newjob_read_control) Call warning('this may lead to a build up of the COM momentum ' &
      ...
      Else If (word(1:3) == 'ttm') Then
        ...
        If (.not. ttm%l_ttm) Then
          ttm%l_ttm = .true.
          If(flow%newjob_read_control) Call info('Two Temperature Model (TTM) opted for',.true.)
        End If
        ...
        Else If (word1(1:5) == 'metal') Then
          ...
          If(flow%newjob_read_control) Call info('electronic subsystem represents metal: thermal conductivity required',.true.)

        Else If (word1(1:8) == 'nonmetal') Then

          ! sets properties of electronic subsystem as a non-metal

          If(flow%newjob_read_control) Call info('electronic subsystem represents non-metal: thermal diffusivity required',.true.)
        ...
        Else If (word1(1:7) == 'ceconst') Then
          ...
          If(flow%newjob_read_control) Call info('electronic specific heat capacity set to constant value',.true.)
        ...
        Else If (word1(1:5) == 'keinf') Then
          ...
          If(flow%newjob_read_control) Call info('electronic thermal conductivity set to infinity',.true.)
        ...
        Else If (word1(1:4) == 'detab') Then
          ...
          If(flow%newjob_read_control) Call info('electronic thermal diffusivity given as tabulated function of temperature',.true.)
        ...
        Else If (word1(1:7) == 'dyndens') Then
          ...
          ttm%ttmdyndens = .true.
          If(flow%newjob_read_control) Call info('dynamic calculations of average atomic density in active ionic cells',.true.)
        ...
        Else If (word1(1:5) == 'sflat') Then
          ...
          ttm%sdepoType = 2
          If(flow%newjob_read_control) Call info('initial homogeneous (flat) spatial energy deposition in electronic system',.true.)
        ...
        Else If (word1(1:5) == 'delta') Then
          ...
          ttm%tdepoType = 3
          If(flow%newjob_read_control) Call info('dirac delta temporal energy deposition in electronic system',.true.)
        ...
        Else If (word1(1:5) == 'pulse') Then
          ...
          If (ttm%tdepo<=zero_plus) Then
            ttm%tdepoType = 3
            If(flow%newjob_read_control) Call info('square pulse temporal energy deposition in electronic' &
        ...
        Else If (word1(1:3) == 'bcs') Then
          ...
          If (word(1:8) == 'periodic') Then
            ttm%bcTypeE = 1
            If(flow%newjob_read_control) Call info('electronic temperature boundary conditions set as periodic',.true.)
        ...
        Else If (word1(1:6) == 'ttm%oneway') Then
          ...
          ttm%oneway = .true.
          If(flow%newjob_read_control) Call info('one-way electron-phonon coupling option switched on',.true.)
      ...
      Else If (word(1:4) == 'defe') Then
        ...
        If (word(1:5) == 'extra') Then
          dfcts(2)%ldef=.true.
          If(flow%newjob_read_control) Call info('defects1 file option on',.true.)
...
2000 Continue
     ...
     If (flow%restart_key == RESTART_KEY_CLEAN) Then
       If(flow%newjob_read_control) Call info('clean start requested',.true.)
    ...
    If (ttm%l_ttm .and. thermo%ensemble/=ENS_NVT_LANGEVIN_INHOMO) Then
      ...
      If (ttm%ttmthvel) Then
        If(flow%newjob_read_control) Call info('applying to thermal velocities in all directions',.true.)
      Else If (ttm%ttmthvelz) Then
        ...
        End If  
      Else
        If(flow%newjob_read_control) Call info('applying to total velocities in all directions',.true.)
      End If
      thermo%ensemble = ENS_NVT_LANGEVIN_INHOMO
    ...
    If (electro%no_elec) Then
      electro%key=ELECTROSTATIC_NULL
      If(flow%newjob_read_control) Call info('Electrostatics switched off!!!',.true.)
    Else If (electro%key == ELECTROSTATIC_NULL) Then
      If(flow%newjob_read_control) Call info('Electrostatics : None Assumed',.true.)
    End If
    ...
    If (electro%key /= ELECTROSTATIC_NULL) Then
      If (electro%lecx) Then
        If(flow%newjob_read_control) Call info('Extended Coulombic eXclusion : YES',.true.)
      Else
        If(flow%newjob_read_control) Call info('Extended Coulombic eXclusion : NO',.true.)
      End If
    End If
    ...
    If (vdws%no_vdw) Then
      If(flow%newjob_read_control) Call info('vdw potential terms switched off',.true.)
    End If
    ...
    If (.not.config%l_vom .and. .not.ttm%l_ttm) Then
      If(flow%newjob_read_control) Call info('no vom option on - COM momentum removal will be abandoned',.true.)
    ...
    Else If (config%l_vom .and. ttm%l_ttm) Then
      If(flow%newjob_read_control) Call info('no vom option off - COM momentum removal will be used',.true.)
    ...
    If (stats%lpana .or. config%mxgana > 0) Then
      If (config%mxgana == 0) Then
        If(flow%newjob_read_control) Call info('no intramolecular distribution collection requested',.true.)
      Else
        If (bond%bin_pdf > 0 .and. angle%bin_adf > 0 .and. &
          ...
          End If  
        Else
          If(flow%newjob_read_control) Call info('intramolecular distribution collection requested for:',.true.)
        End If
      ...
      If (stats%lpana) Then
        If(flow%newjob_read_control) Call info('probability distribution analysis printing requested',.true.)
      Else
        If(flow%newjob_read_control) Call info('no probability distribution analysis printing requested',.true.)
      End If
    End If
    ...
    If (rdf%l_collect .or. rdf%l_print) Then
      If (rdf%l_collect) Then
        ...
        If(flow%newjob_read_control) Call info(messages,3,.true.)
      Else
        If(flow%newjob_read_control) Call info('no rdf collection requested',.true.)
      End If

      If (rdf%l_print) Then
        If(flow%newjob_read_control) Call info('rdf printing requested',.true.)
      Else
        If (stats%lpana) Then
          If(flow%newjob_read_control) Call info('rdf printing triggered due to a PDA printing request',.true.)
          rdf%l_print=stats%lpana
        Else
          If(flow%newjob_read_control) Call info('no rdf printing requested',.true.)
        End If
      End If

      If (rdf%max_rdf == 0) Then
        If(flow%newjob_read_control) Call info('no rdf pairs specified in FIELD',.true.)
      Else
        If(flow%newjob_read_control) Call info('rdf pairs specified in FIELD',.true.)
      End If

      If ((.not.rdf%l_collect) .or. rdf%max_rdf == 0) Then
        If(flow%newjob_read_control) Call info('rdf routines not to be activated',.true.)
        rdf%l_collect=.false.
        rdf%l_print=.false.
      End If
    End If
    ...
    If (zdensity%l_collect .or. zdensity%l_print) Then
      If (zdensity%l_collect) Then
        ...
        If(flow%newjob_read_control) Call info(messages,3,.true.)
      Else
        If(flow%newjob_read_control) Call info('no z-density profiles requested',.true.)
      End If

      If (zdensity%l_print) Then
        If(flow%newjob_read_control) Call info('z-density printing requested',.true.)
      Else
        If(flow%newjob_read_control) Call info('no z-density printing requested',.true.)
      End If

      If (.not.zdensity%l_collect) Then
        If(flow%newjob_read_control) Call info('z-density routines not to be activated',.true.)
        zdensity%l_print=.false.
      End If
    End If
    ...
    If (green%samp > 0 .or. green%l_print) Then
      If (green%samp > 0) Then
        ...
        If(flow%newjob_read_control) Call info(messages,3,.true.)
      Else
        If(flow%newjob_read_control) Call info('no vaf collection requested',.true.)
      End If

      If (green%l_print) Then
        If(flow%newjob_read_control) Call info('vaf printing requested',.true.)
      Else
        If(flow%newjob_read_control) Call info('no vaf printing requested',.true.)
      End If

      If (green%l_average) Then
        If(flow%newjob_read_control) Call info('time-averaged vaf profile',.true.)
      Else
        If(flow%newjob_read_control) Call info('instantaneous vaf profiles',.true.)
      End If
    End If
    ...
    If (.not.flow%simulation) Then
      ...
      If (flow%restart_key /= RESTART_KEY_CLEAN) Then
        ...
        If(flow%newjob_read_control) Call info('clean start enforced',.true.)
      End If
    End If
    ...
    If (thermo%ensemble == ENS_NVT_LANGEVIN_INHOMO ) Then
      ...
      If (Abs(thermo%chi_es) <= zero_plus) Then
        If(flow%newjob_read_control) Call info('assuming no electronic stopping in inhomogeneous Langevin thermostat',.true.)
      End If
    End If
...
! read termination
    ...
    Else If (word(1:2) == 'no') Then
      ...
      Else If (word(1:3) == 'ind' ) Then
        ...
        l_ind=.false.
        If(flow%newjob_set_bounds) Call info('no index (reading in CONFIG) option on',.true.)
  ...
  If (la_ana) Then
    ...
    If (la_bnd) Then
      ...
      If(flow%newjob_set_bounds) Call warning('bond PDF analisys cutoff check: bond%rcut=Max(bond%rcut,rcbnd_def)',.true.)
      If(flow%newjob_set_bounds) Call warning(40,bond%rcut,0.0_wp,0.0_wp)
    End If
  ...
  If (lvdw) Then
    If (.not.lrvdw) Then
      ...
      If(flow%newjob_set_bounds) Call warning( &
        'short-ranged interaction cutoff check: vdws%cutoff = Min(vdws%cutoff,Max(neigh%cutoff,rcut_def))', &
        .true.)
      If(flow%newjob_set_bounds) Call warning(40,vdws%cutoff,0.0_wp,0.0_wp)
    End If
  ...
  If(flow%newjob_set_bounds) Call warning( &
    ...
    .true.)
  If(flow%newjob_set_bounds) Call warning(40,neigh%cutoff,0.0_wp,0.0_wp)
    ...
    Else If (word(1:6) == 'finish') Then
      ...
      If ((.not.lrmet) .and. lmet) Then
        If (lrcut .or. lrvdw) Then
          ...
          If(flow%newjob_set_bounds) Call warning('metal interaction cutoff check: met%rcut=Max(neigh%cutoff,vdws%cutoff)',.true.)
          If(flow%newjob_set_bounds) Call warning(40,met%rcut,0.0_wp,0.0_wp)
        Else
          Call error(382)
        End If
      End If
      ...
      If (ewld%bspline /= 0) Then ! SPME or Poisson Solver
        ...
        If (.not.lrcut) Then
          ...
          neigh%cutoff=Max( neigh%cutoff, Merge(Real(ewld%bspline,wp)/electro%alpha,                          &
            ...
            itmp == 0) )
          If(flow%newjob_set_bounds) Call warning('DD cutoff check: rcut compliance with Ewald summation' // &
        ...
        If ( ((.not.lrcut) .or. (.not.flow%strict)) .and. &
          ...
          If (max_rigid == 0) Then ! compensate for Max(Size(RBs))>vdws%cutoff
            neigh%cutoff=Max(vdws%cutoff,met%rcut,kim_data%cutoff,bond%rcut, &
              2.0_wp*rcter+1.0e-6_wp)
          If(flow%newjob_set_bounds) Call warning('DD cutoff check: neigh%cutoff = Max(vdws%cutoff,'// &
          ...
          Else
            ...
            If(flow%newjob_set_bounds) Call warning('DD cutoff check: neigh%cutoff = Max(neigh%cutoff,vdws%cutoff,'// &
              'met%rcut,kim_data%cutoff,bond%rcut,2.0_wp*rcter+1.0e-6_wp', &
              .true.)
            If(flow%newjob_set_bounds) Call warning(40,neigh%cutoff,0.0_wp,0.0_wp)
          End If
        ...
        If (lter .and. electro%no_elec .and. vdws%no_vdw .and. l_n_m .and. l_n_r .and. &
          ...
          If (.not.flow%strict) Then
            ...
            If (max_rigid == 0) Then ! compensate for Max(Size(RBs))>vdws%cutoff
              ...
              If(flow%newjob_set_bounds) Call warning('DD cutoff check: neigh%cutoff = Max('// &
                'bond%rcut,2.0_wp*rcter+1.0e-6_wp',.true.)
              If(flow%newjob_set_bounds) Call warning(40,neigh%cutoff,0.0_wp,0.0_wp)
            Else
              neigh%cutoff=Max(neigh%cutoff,bond%rcut,2.0_wp*rcter+1.0e-6_wp)
              If(flow%newjob_set_bounds) Call warning('DD cutoff check: neigh%cutoff = Max(neigh%cutoff'// &
                'bond%rcut,2.0_wp*rcter+1.0e-6_wp',.true.)
             If(flow%newjob_set_bounds) Call warning(40,neigh%cutoff,0.0_wp,0.0_wp)
            End If
          End If
      ...
      If (lmet) Then
        ...
        If(flow%newjob_set_bounds) Call warning('metal interactions cutoff check: met%rcut = neigh%cutoff',.true.)
        If(flow%newjob_set_bounds) Call warning(40,met%rcut,0.0_wp,0.0_wp)
      End If
      ...
      If (lvdw .and. vdws%cutoff > neigh%cutoff) Then
        vdws%cutoff = neigh%cutoff
        If(flow%newjob_set_bounds) Call warning('short-ranged interactions cutoff check: vdws%cutoff = neigh%cutoff',.true.)
        If(flow%newjob_set_bounds) Call warning(40,vdws%cutoff,0.0_wp,0.0_wp)
      End If
      ...
      If (rdf%rbin < 1.0e-05_wp .or. rdf%rbin > neigh%cutoff/4.0_wp) Then
        ...
        If(flow%newjob_set_bounds) Call warning('bin size cutoff check: rdf%rbin = Min(rbin_def,neigh%cutoff/4.0_wp)',.true.)
        If(flow%newjob_set_bounds) Call warning(40,rdf%rbin,0.0_wp,0.0_wp)
      End If
