...
Contains
  ...
  Subroutine read_control(control_file, params, comm, is_old)
    !!-----------------------------------------------------------------------
    !!
    ...
    is_old = is_old_control(control_file%unit_no, comm)
  ...
  Function is_old_control(ifile, comm) Result(old_control)
    !!-----------------------------------------------------------------------
    !!
    ...
    Do
      ...
      If (key == "") Cycle
      Call lower_case(key)
      old_control = .not. (key == "title")
      ! can_parse = params%in(key)
      Exit
    End Do
    Rewind (ifile)

  End Function is_old_control
