...
Subroutine vnl_check(l_str,rcut,rpad,rlnk,width,parts,comm)
...
! Checks
  ...
  Do i=1,nlast
    x(i) = parts(i)%xxx - xbg(i)
    y(i) = parts(i)%yyy - ybg(i)
    z(i) = parts(i)%zzz - zbg(i)
  End Do
...
Subroutine vnl_set_check(parts,comm)
