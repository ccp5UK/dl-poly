...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Real( Kind = wp ), Intent( In    ) :: quattol
...
! split anisotropic from semi-isotropic barostats (iso=0,1,2,3)
  ...
  If (iso == 0) Then
     eta1=(eta + tstep*( strcom+stress+strkin + fac*uni - &
                         (press*uni+strext)*volm )/pmass)*Exp(-tstep*chit2)
  Else
     If      (iso == 2) Then
        eta1(1)=(eta(1) + tstep*( strcom(1)+stress(1)+strkin(1) + fac - &
                                  (press+strext(1)-ten/h_z)*volm )/pmass)*Exp(-tstep*chit2)
        eta1(5)=(eta(5) + tstep*( strcom(5)+stress(5)+strkin(5) + fac - &
     ...
     Else If (iso == 3) Then
        ...
        eta1(5)=eta1(1)
     End If
     eta1(9)=(eta(9) + tstep*( strcom(9)+stress(9)+strkin(9) + fac - &
