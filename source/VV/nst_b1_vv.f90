...
! split anisotropic from semi-isotropic barostats (iso=0,1,2)
  ...
  eta(6:8)=0.0_wp
           Else If (iso == 2) Then
  eta(1)=1.0_wp + tstep*beta*(strcom(1)+strcon(1)+strpmf(1)+stress(1)+strkin(1)-(press+strext(1)-ten/h_z)*volm)/(taup*volm)
  eta(2:4)=0.0_wp
  eta(5)=1.0_wp + tstep*beta*(strcom(5)+strcon(5)+strpmf(5)+stress(5)+strkin(5)-(press+strext(5)-ten/h_z)*volm)/(taup*volm)
  eta(6:8)=0.0_wp
           End If
...
! Get RB COM stress and virial
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nst_b1 deallocation failure, node: ', idnode
     Call error(0)
  End If
