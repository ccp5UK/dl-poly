...
! molecular dynamics - leapfrog verlet Nose-Hoover thermostat and
! barostat (isotropic pressure control)
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use kinetic_module, Only : getcom,getvom,getkin,kinstress
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'npt_h0 allocation failure, node: ', idnode
     Call error(0)
  End If
...
! update kinetic energy and stress at full step
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'npt_h0 deallocation failure, node: ', idnode
     Call error(0)
  End If

End Subroutine npt_h0_lfv
