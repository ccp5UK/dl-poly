...
! Get to real space
        ...
        Do jdnode=0,mxnode-1
           If (jdnode > 0) Then
              Call MPI_SEND(ready,1,MPI_LOGICAL,jdnode,DefRWrite_tag,dlp_comm_world,ierr)

              Call MPI_RECV(jatms,1,MPI_INTEGER,jdnode,DefRWrite_tag,dlp_comm_world,status,ierr)
              If (jatms > 0) Then
                 Call MPI_RECV(chbuf,8*jatms,MPI_CHARACTER,jdnode,DefRWrite_tag,dlp_comm_world,status,ierr)
                 Call MPI_RECV(iwrk,jatms,MPI_INTEGER,jdnode,DefRWrite_tag,dlp_comm_world,status,ierr)

                 Call MPI_RECV(axx,jatms,wp_mpi,jdnode,DefRWrite_tag,dlp_comm_world,status,ierr)
                 Call MPI_RECV(ayy,jatms,wp_mpi,jdnode,DefRWrite_tag,dlp_comm_world,status,ierr)
                 Call MPI_RECV(azz,jatms,wp_mpi,jdnode,DefRWrite_tag,dlp_comm_world,status,ierr)
              End If
...
! Close REFERENCE
     ...
     Else
        ...
        Call MPI_RECV(ready,1,MPI_LOGICAL,0,DefRWrite_tag,dlp_comm_world,status,ierr)

        Call MPI_SEND(nrefs,1,MPI_INTEGER,0,DefRWrite_tag,dlp_comm_world,ierr)
        If (nrefs > 0) Then
           Call MPI_SEND(namr,8*nrefs,MPI_CHARACTER,0,DefRWrite_tag,dlp_comm_world,ierr)
           Call MPI_SEND(indr,nrefs,MPI_INTEGER,0,DefRWrite_tag,dlp_comm_world,ierr)

           Call MPI_SEND(xr,nrefs,wp_mpi,0,DefRWrite_tag,dlp_comm_world,ierr)
           Call MPI_SEND(yr,nrefs,wp_mpi,0,DefRWrite_tag,dlp_comm_world,ierr)
           Call MPI_SEND(zr,nrefs,wp_mpi,0,DefRWrite_tag,dlp_comm_world,ierr)
        End If
