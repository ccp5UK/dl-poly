...
! uni is the diagonal unit matrix
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nst_b0 allocation failure, node: ', comm%idnode
     Call error(0)
  End If
...
! update kinetic energy and stress
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nst_b0 deallocation failure, node: ', comm%idnode
     Call error(0)
  End If
...
! uni is the diagonal unit matrix
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nst_b1 allocation failure, node: ', comm%idnode
     Call error(0)
  End If
...
! update kinetic energy and stress
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nst_b1 deallocation failure, node: ', comm%idnode
     Call error(0)
  End If
