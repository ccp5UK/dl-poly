...
Contains
  ...
  Subroutine analysis_result(lrdf,lpana,lprdf, &
                             ...
                             bond,angle,dihedral,inversion,stats,green,zdensity,neigh,site_data,comm)
    ...
    Do i = 1,site_data%ntype_atom
      site_data%dens(i) = site_data%dens(i)*(volm/avvol)
    End Do
    ...
    If (lrdf .and. lprdf .and. ncfrdf > 0 .and. l_errors_block) Then
      Call calculate_errors(temp,rcut,nstep,neigh,site_data,comm)
    End If
    If (lrdf .and. lprdf .and. ncfrdf > 0 .and. l_errors_jack .and. .not. l_errors_block) Then
      Call calculate_errors_jackknife(temp,rcut,nstep,neigh,site_data,comm)
    End If
    If (lrdf .and. lprdf .and. ncfrdf > 0 .and. .not.(l_errors_block .or. l_errors_jack)) Then
      Call rdf_compute(lpana,rcut,temp,site_data,comm)
    End IF
    ...
    If (zdensity%l_collect .and. zdensity%l_print .and. zdensity%n_samples > 0) Then
      Call z_density_compute(zdensity,site_data,comm)
    End If

    ! calculate and print velocity autocorrelation function
    If (green%samp > 0 .and. green%l_print .and. green%vafcount > zero_plus) Then
      Call vaf_compute(tstep,site_data%num_type_nf,green,comm)
    End If

    ! Calculate and print PDFs
    If (lpana) Then
      If (bond%bin_pdf > 0 .and. bond%n_frames > 0) Then
        Call bonds_compute(temp,site_data%unique_atom,bond,comm)
      End If
      If (angle%bin_adf > 0 .and. angle%n_frames > 0) Then
        Call angles_compute(temp,site_data%unique_atom,angle,comm)
      End If
      If (dihedral%bin_adf > 0 .and. dihedral%n_frames > 0) Then
        Call dihedrals_compute(temp,site_data%unique_atom,dihedral,comm)
      End If
      If (inversion%bin_adf > 0 .and. inversion%n_frames > 0) Then
        Call inversions_compute(temp,site_data%unique_atom,inversion,comm)
      End If
    End If
