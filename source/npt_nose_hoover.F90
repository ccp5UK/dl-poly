...
Contains
  ...
  ! integrate and apply npt_h0_scl barostat - 1/2 step
          ...
          Call npt_h0_scl &
             (0,hstep,degfre,thermo%pmass,thermo%chi_t,volm,vir,virtot, &
  ...
  ! update positions
          ...
          Do i=1,natms
             If (weight(i) > 1.0e-6_wp) Then
                parts(i)%xxx=scale*(xxt(i)-com(1))+tstep*vxx(i)+com(1)
                parts(i)%yyy=scale*(yyt(i)-com(2))+tstep*vyy(i)+com(2)
                parts(i)%zzz=scale*(zzt(i)-com(3))+tstep*vzz(i)+com(3)
             End If
          End Do
...
If ( adjust_timestep(tstep,hstep,rstep,qstep,mndis,mxdis,mxstp,natms,parts,&
 xxt,yyt,zzt,cshell%legshl,message,mxdr,comm)) Then 
  ...
  ! integrate and apply npt_h1_scl barostat - 1/2 step
          ...
          Call npt_h1_scl &
             (0,hstep,degfre,degrot,thermo%pmass,thermo%chi_t,volm,vir,virtot,vircom, &
