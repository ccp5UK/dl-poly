Module nst_berendsen
  ...
  Use setup,           Only : mxpmf,mxtpmf,mxatyp,zero_plus
...
Contains
  ...
  ! calculate Berendsen barostat: thermo%eta, iterate stat%strcon and strpmf

  ! split anisotropic from semi-isotropic barostats (thermo%iso=0,1,2,3)

          If (thermo%iso == 0) Then
             thermo%eta=uni + tstep*beta*(stat%strcon+strpmf+stress+strkin- &
               (thermo%press*uni+thermo%stress)*volm)/(thermo%tau_p*volm)
          Else
             If      (thermo%iso == 2) Then
                thermo%eta(1)=1.0_wp + tstep*beta*(stat%strcon(1)+strpmf(1)+stress(1)+strkin(1)- &
                  (thermo%press+thermo%stress(1)-thermo%tension/h_z)*volm)/(thermo%tau_p*volm)
                thermo%eta(5)=1.0_wp + tstep*beta*(stat%strcon(5)+strpmf(5)+stress(5)+strkin(5)- &
             ...
             Else If (thermo%iso == 3) Then
                thermo%eta(1)=1.0_wp + tstep*beta*( 0.5_wp*                                                       &
                       (stat%strcon(1)+strpmf(1)+stress(1)+strkin(1)+stat%strcon(5)+strpmf(5)+stress(5)+strkin(5)) - &
                       (thermo%press+0.5_wp*(thermo%stress(1)+thermo%stress(5))-thermo%tension/h_z)*volm ) &
                       / (thermo%tau_p*volm)
                thermo%eta(5)=thermo%eta(1)
             End If
             thermo%eta(9)=1.0_wp + tstep*beta*(stat%strcon(9)+strpmf(9)+stress(9)+strkin(9)- &
  ...
  ! update cell parameters: anisotropic
       ...
       If (cons%megcon == 0 .and. megpmf == 0) Call mat_mul(thermo%eta,czero,cell)
  ...
  ! split anisotropic from semi-isotropic barostats (thermo%iso=0,1,2,3)
          ...
          If (thermo%iso == 0) Then
             thermo%eta=uni + tstep*beta*(strcom+stat%strcon+strpmf+stress+strkin- &
               (thermo%press*uni+thermo%stress)*volm)/(thermo%tau_p*volm)
          Else
             If      (thermo%iso == 2) Then
                thermo%eta(1)=1.0_wp + tstep*beta*(strcom(1)+stat%strcon(1)+strpmf(1)+stress(1)+strkin(1) - &
                  (thermo%press+thermo%stress(1)-thermo%tension/h_z)*volm)/(thermo%tau_p*volm)
                thermo%eta(5)=1.0_wp + tstep*beta*(strcom(5)+stat%strcon(5)+strpmf(5)+stress(5)+strkin(5) - &
                  (thermo%press+thermo%stress(5)-thermo%tension/h_z)*volm)/(thermo%tau_p*volm)
             Else If (thermo%iso == 3) Then
                thermo%eta(1)=1.0_wp + tstep*beta*( 0.5_wp* &
                  (strcom(1)+stat%strcon(1)+strpmf(1)+stress(1)+strkin(1)  + &
                   strcom(5)+stat%strcon(5)+strpmf(5)+stress(5)+strkin(5)) - &
                  (thermo%press+0.5_wp*(thermo%stress(1)+thermo%stress(5))-thermo%tension/h_z)*volm ) &
                  / (thermo%tau_p*volm)
                thermo%eta(5)=thermo%eta(1)
             End If
             thermo%eta(9)=1.0_wp + tstep*beta*(strcom(9)+stat%strcon(9)+strpmf(9)+stress(9)+strkin(9)- &
               (thermo%press+thermo%stress(9))*volm)/(thermo%tau_p*volm)
          End If
