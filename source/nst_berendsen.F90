Module nst_berendsen
  ...
  Use setup,           Only : mxcons,mxpmf,mxtpmf,mxatyp,zero_plus
...
Contains
  ...
  ! uni is the diagonal unit matrix
    ...
    If (Any(fail > 0)) Then
       Write(message,'(/,1x,a)') 'nst_b0 allocation failure'
  ...
  ! update kinetic energy and stress
    ...
    If (Any(fail > 0)) Then
       Write(message,'(/,1x,a)') 'nst_b0 deallocation failure'
       Call error(0,message)
    End If
  ...
  ! uni is the diagonal unit matrix
    ...
    If (Any(fail > 0)) Then
       Write(message,'(/,1x,a)') 'nst_b1 allocation failure'
       Call error(0,message)
    End If
  ...
  ! update kinetic energy and stress
    ...
    If (Any(fail > 0)) Then
       Write(message,'(/,1x,a)') 'nst_b1 deallocation failure'
       Call error(0,message)
    End If
