Module io
  ...
  Use mpi_api
#else
  Use mpi, only : MPI_COMM_NULL, MPI_COMM_SPLIT, MPI_COMM_FREE, MPI_UNDEFINED,&
    MPI_TYPE_CONTIGUOUS,MPI_CHARACTER, MPI_INFO_NULL, MPI_INTEGER,MPI_GATHERV,&
    MPI_ALLTOALLV,MPI_FILE_SET_VIEW, MPI_ALLREDUCE, MPI_ALLTOALL,MPI_OFFSET_KIND,&
    MPI_ALLREDUCE,MPI_SUM, MPI_STATUS_SIZE, MPI_MAX, MPI_MIN, MPI_LAND, MPI_LOGICAL
#endif
  ...
  Type, Public :: io_type
    ...
    Integer          :: rec_type
    Type(file_data)  :: known_files(LOW_HANDLE:HIGH_HANDLE)
...
! Finalize the sorted I/O method for MPI-I/O
  ...
  Subroutine io_set_parameters(io, user_comm, user_method_write, user_method_read, user_n_io_procs_write, user_n_io_procs_read, &
    ...
    Type(io_type),       Intent(InOut) :: io
    Integer, Optional,   Intent(In   ) :: user_comm, user_method_write, user_method_read, &
                                          user_n_io_procs_write, user_n_io_procs_read, &
                                          user_batch_size_write, user_batch_size_read, &
                                          user_buffer_size_write, user_buffer_size_read
    Character, Optional, Intent(In   ) :: user_line_feed
    Logical, Optional,   Intent(In   ) :: user_error_check
  ...
  Subroutine io_get_parameters(io, user_comm, user_method_write, user_method_read, user_n_io_procs_write, user_n_io_procs_read, &
    ...
    Type(io_type),       Intent(InOut) :: io
    Integer, Optional,   Intent(  Out) :: user_comm, user_method_write, user_method_read, &
                                          user_n_io_procs_write, user_n_io_procs_read, &
                                          user_batch_size_write, user_batch_size_read, &
                                          user_buffer_size_write, user_buffer_size_read
    Character, Optional, Intent(  Out) :: user_line_feed
    Logical, Optional,   Intent(  Out) :: user_error_check
  ...
  Contains
    ...
    Integer,                           Intent(In   ) :: write_level, write_options
    Integer, Dimension(:),             Intent(In   ) :: global_index_rank, global_indices
    Type(corePart), Dimension(:),      Intent(In   ) :: parts
    Real(Kind=wp), Dimension(:),       Intent(In   ) :: vx, vy, vz, weight, rsd
    Character(Len=*), Dimension(:),    Intent(In   ) :: atom_name
    ...
    Integer                            :: i, index, j, n, p, q, temp_scal
    ...
    Integer,                           Intent(In   ) :: io_gather_comm, local_bottom, local_top
    ...
    Integer,                           Intent(  Out) :: n_gathered, error

    Integer                            :: i, me_in_gather, n_atoms, n_data, n_procs_gather, &
                                          this_size
    Integer, Allocatable, Dimension(:) :: displs_for_gather, n_to_gather
    ...
    Character(Len=1), Allocatable, Dimension(:, :) :: nm
    Integer                                        :: i, nd, nn
    Integer, Allocatable, Dimension(:)             :: gi, ranks
    Real(Kind=wp), Allocatable, Dimension(:, :)    :: dt
    ...
    Integer,                Intent(In   ) :: io_comm, n
    Integer, Dimension(0:), Intent(  Out) :: n_reorg

    Integer :: n_av, n_in_io_comm, n_left, n_total
    ...
    Integer                            :: at1, first_in_batch, i, j, leng, me_in_io_comm, &
                                          my_first, n_in_io_comm, ndat
    Integer, Allocatable, Dimension(:) :: displs_recv, displs_send, first_atom, to_recv, to_send
    ...
    Type(io_type),                     Intent(InOut) :: io
    Integer,                           Intent(In   ) :: io_comm, write_level, write_options, &
                                                        file_handle
    Integer(Kind=MPI_OFFSET_KIND),     Intent(In   ) :: first_record
    Integer,                           Intent(In   ) :: n
    Integer, Dimension(:),             Intent(In   ) :: indices
  ...
  Contains
    ...
    Integer,                           Intent(In   ) :: write_level, write_options
    Integer, Dimension(:),             Intent(In   ) :: global_index_rank, global_indices
    Real(Kind=wp), Dimension(:),       Intent(In   ) :: rx, ry, rz, vx, vy, vz, fx, fy, fz, &
                                                        weight, charge, rsd
    Character(Len=*), Dimension(:),    Intent(In   ) :: atom_name
    Integer, Dimension(:),             Intent(  Out) :: local_global_indices
    Real(Kind=wp), Dimension(:, :),    Intent(  Out) :: local_data
  ...
  Subroutine io_write_batch(io, io_file_handle, next_rec, in_buffer, buffer)
    ...
    Integer :: file_handle, i, ierr, status(1:MPI_STATUS_SIZE)
  ...
  Subroutine io_read_batch(io, io_file_handle, next_rec, in_buffer, buffer, error)
    ...
    Integer :: count, file_handle, i, ierr, status(1:MPI_STATUS_SIZE)
  ...
  Subroutine split_io_comm(base_comm, n_io_procs_write, io_comm, io_gather_comm, do_io)
    ...
    Integer, Intent(In   ) :: base_comm, n_io_procs_write
    Integer, Intent(  Out) :: io_comm, io_gather_comm
    Logical, Intent(  Out) :: do_io

    Integer :: actual_io_procs, colour, everyth_for_io, ierr, key, n_base, rank_base
...
! intent Out, on procs doing I/O it is InOut
  ...
  Subroutine batch_limits_set(local_global_indices, bottom_batch, top_batch, &
    ...
    Integer, Dimension(:), Intent(In   ) :: local_global_indices
    Integer,               Intent(In   ) :: bottom_batch, top_batch
    Integer,               Intent(  Out) :: local_bottom, local_top
