...
# INTER-(RDF) & INTRA-(bonded)MOLECULAR PDF ANALYSIS
print analysis
\end{verbatim}
...
and only when the {\bf rdf} option is active then the RDFPMF and
RDFTAB files will be produced, along with RDFDAT.  They are
structured in the same manner and format as their
intramolecular counterparts.  The user can then convert the RDFTAB
file into a correctly formatted TABLE file by using the utility called
