Subroutine ewald_excl_mfield_d &
           (iatm,rcut,alpha,epsq,xxt,yyt,zzt,rrt)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! dl_poly_4 subroutine for calculating the electrostatic field
! in a periodic system due to permanent multipoles with the ewald real
! space kernel
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Integer           :: fail,idi,jatm,limit,k,m

  Real( Kind = wp ) :: scl,rsq,rrr,fix,fiy,fiz,fx,fy,fz,        &
                       talp2,alpr,alpr2,alpsqrpi,               &
                       tix,tiy,tiz,tjx,tjy,tjz,                 &
                       tt,b0,b1,b2,b3,b4,b5,exparr,bijmp,       &
...
! electrostatic field due to dipole on atom i
              ...
              If (mxompl >= 1) Then
                 ...
                 tix = tix - (dpxx*imp2 + dpxy*imp3 + dpxz*imp4)
                 tiy = tiy - (dpxy*imp2 + dpyy*imp3 + dpyz*imp4)
                 tiz = tiz - (dpxz*imp2 + dpyz*imp3 + dpzz*imp4)
...
! electrostatic field due to quadrupole on atom i
              ...
              If (mxompl == 2) Then
                 ...
                 tix = tix - (imp5*dpxxx+imp6*dpxxy+imp7*dpxxz+imp8*dpxyy+imp9*dpxyz+imp10*dpxzz)
                 tiy = tiy - (imp5*dpxxy+imp6*dpxyy+imp7*dpxyz+imp8*dpyyy+imp9*dpyyz+imp10*dpyzz)
                 tiz = tiz - (imp5*dpxxz+imp6*dpxyz+imp7*dpxzz+imp8*dpyyz+imp9*dpyzz+imp10*dpzzz)

              End If

              mpfldx(jatm) = mpfldx(jatm) + tix
              mpfldy(jatm) = mpfldy(jatm) + tiy
              mpfldz(jatm) = mpfldz(jatm) + tiz
...
End Subroutine ewald_excl_mfield_d
