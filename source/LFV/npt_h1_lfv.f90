...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use kinetic_module,     Only : getcom,getvom,getknr,getknf,getknt, &
                                 kinstresf,kinstrest
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'npt_h1 allocation failure, node: ', idnode
     Call error(0)
  End If
...
! and barostating in leapfrog verlet scheme - FPs
     ...
     Do j=1,nfree
        ...
        If (weight(i) > 1.0e-6_wp) Then
           ...
           vxx(i)=vxt(i)+tstep*(tmp*fxx(i)-(chit2+chip2)*uxt(i))
           vyy(i)=vyt(i)+tstep*(tmp*fyy(i)-(chit2+chip2)*uyt(i))
           vzz(i)=vzt(i)+tstep*(tmp*fzz(i)-(chit2+chip2)*uzt(i))
...
! advance RB COM velocity by leapfrog
           ...
           rgdvxx(irgd)=rgdvxt(irgd)+tstep*(tmp*fmx-(chit2+chip2)*rgdvxo(irgd))
           rgdvyy(irgd)=rgdvyt(irgd)+tstep*(tmp*fmy-(chit2+chip2)*rgdvyo(irgd))
           rgdvzz(irgd)=rgdvzt(irgd)+tstep*(tmp*fmz-(chit2+chip2)*rgdvzo(irgd))
...
! update kinetic energy and stress at full step
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'npt_h1 deallocation failure, node: ', idnode
     Call error(0)
  End If

End Subroutine npt_h1_lfv
