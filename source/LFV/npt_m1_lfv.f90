...
! - leapfrog verlet with Nose-Hoover thermostat and barostat (isotropic
! pressure control) and MTK coupling (symplectic)
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'npt_m1 allocation failure, node: ', idnode
     Call error(0)
  End If
...
! (vircon,virpmf,chit2,chip2 are zero!!!)
  ...
  chip1 = chip + tstep*((2.0_wp*(1.0_wp+factor)*engke-virtot-vircom) - 3.0_wp*press*vzero)/pmass
...
! advance RB COM velocity by leapfrog
           ...
           rgdvxx(irgd)=rgdvxt(irgd)+tstep*(tmp*fmx-(chit2+(1.0_wp+factor)*chip2)*rgdvxo(irgd))
           rgdvyy(irgd)=rgdvyt(irgd)+tstep*(tmp*fmy-(chit2+(1.0_wp+factor)*chip2)*rgdvyo(irgd))
           rgdvzz(irgd)=rgdvzt(irgd)+tstep*(tmp*fmz-(chit2+(1.0_wp+factor)*chip2)*rgdvzo(irgd))
...
! (vircon,virpmf,chit2,chip2 are freshly new here!!!)
        ...
        chip1 = chip*Exp(-tstep*chit2) +                                             &
                tstep*( (2.0_wp*(1.0_wp+factor)*engke-virtot-vircon-virpmf-vircom) - &
...
! update kinetic energy and stress at full step
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'npt_m1 deallocation failure, node: ', idnode
     Call error(0)
  End If

End Subroutine npt_m1_lfv
