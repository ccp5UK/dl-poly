...
! avoid user defined 0K field to break up anything
  ...
  If (megrgd > 0) Then
     engf=getknf(vxx,vyy,vzz)/Real(Max(1_li,degfre),wp)
     engt=getknt(rgdvxx,rgdvyy,rgdvzz)/Real(Max(1_li,degtra),wp)

     engr=getknr(rgdoxx,rgdoyy,rgdozz)/Real(Max(1_li,degrot),wp)
     engk=engf+engt
  Else
     engk=getkin(vxx,vyy,vzz)/Real(Max(1_li,degfre),wp)
  End If
