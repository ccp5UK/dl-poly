Module rdfs
  ...
  Type, Public :: rdf_type
    ...
    !> RDF step to start collection
    Integer(Kind=wi), Public              :: step_start = 0
...
Contains
  ...
  Subroutine rdf_collect(iatm, rrt, neigh, config, rdf)
    ...
    Integer       :: ai, aj, idi, jatm, keyrdf, kk, ll, m
  ...
  Subroutine calculate_block(temp, rcut, sites, config, rdf)
    ...
    Integer       :: i, ia, ib, kk, ngrid
    Logical       :: zero
    Real(Kind=wp) :: delr, dgrid, dvol, factor1, gofr, gofr1, kT2engo, pdfzero, rdlr, rrr
  ...
  Subroutine rdf_excl_collect(iatm, rrt, neigh, config, rdf)
    ...
    Integer       :: ai, aj, idi, jatm, keyrdf, kk, limit, ll, m
    Real(Kind=wp) :: rdelr, rrr
