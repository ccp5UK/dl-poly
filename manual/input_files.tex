...
\>                                              \> $r_{\rm cut}$, to $f$ \AA~ ($0.05~\ge~f$, default $f~=~0$) \\
\>                                              \> \\
...
10\% to 100\% depending on force-field complexity.  If it is too large or too
small (that is why the $\ge~0.05$~\AA limit) it will lead to performace
degradation.  It is recomended that $r_{\rm pad}$ is set up at a value of
$\approx~1~\div~5\%$ of the cutoff, $r_{\rm cut}$, as long as the major
...
for force-fields involving the Ewald summation methodology and $60~\div~100\%$
...
$ \\
           &                     &       &           &         &         &               & \\
{\bf osel} & Osc. Electric Field & $E_{x}$ & $E_{y}$ & $E_{z}$ & $\omega^{in}_{ps^{-1}}$ & &
