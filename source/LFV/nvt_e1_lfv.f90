...
! - leapfrog verlet with Gaussian temperature
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nvt_e1 allocation failure, node: ', idnode
     Call error(0)
  End If
...
! estimate chit
  ...
  Call nvt_e1_scl &
           (0,tstep,fxt,fyt,fzt,oxt,oyt,ozt,          &
...
! update velocity and position of FPs
     ...
     tmp   = 1.0_wp/(1.0_wp+0.25_wp*chit*tstep)
...
! estimate chit and engke and engrot

     Call nvt_e1_scl &
           (0,tstep,fxx,fyy,fzz,oxt,oyt,ozt,          &
           rgdfxx,rgdfyy,rgdfzz,rgdtxx,rgdtyy,rgdtzz, &
           rgdvxo,rgdvyo,rgdvzo,rgdoxo,rgdoyo,rgdozo, &
           chit,engke,engrot)
...
! update kinetic stress at full step
  ...
  If (Any(fail > 0)) Then
     Write(nrite,'(/,1x,a,i0)') 'nvt_e1 deallocation failure, node: ', idnode
     Call error(0)
  End If

End Subroutine nvt_e1_lfv
