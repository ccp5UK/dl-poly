...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Real( Kind = wp ) :: half_minus,t,cell(1:9),celprp(1:10),rcell(1:9),fx,fy,fz,  &
                       conv,xl,yl,zl,alpha,beta,gamma,cosa,sina,cosb,cosc,z2,z3, &
...
! Read command line
  ...
  Write(*,*)
  Write(*,'(1x,a)') 'Enter space separated input configuration filename and format (0,1,2,3): '
  record=' '
...
! Read command line
  ...
  Write(*,'(1x,a)') 'Enter space separated output format (0,1,2,3) and optional configuration filename: '
  record=' '
  Do While (Len_Trim(record) == 0)
     Write(*,'(1x,a)',Advance='No')
     Read(*,'(a)') record
...
! Create name for the expanded configuration
  ...
  If (Len_Trim(f_out) == 0) Then
     record=' '
     word=' ' ; Write(word,'(i5)') nx ; Call strip_blanks(word) ; record=record(1:Len_Trim(record)) // '_' // word(1:Len_Trim(word))
     word=' ' ; Write(word,'(i5)') ny ; Call strip_blanks(word) ; record=record(1:Len_Trim(record)) // '_' // word(1:Len_Trim(word))
     word=' ' ; Write(word,'(i5)') nz ; Call strip_blanks(word) ; record=record(1:Len_Trim(record)) // '_' // word(1:Len_Trim(word))
     f_out=' '
...
! Write configuration file headers
           ...
           Do While ( .true. )
              ...
              If (type_in == 0 .and. levcfg > 0) Read(Unit=nread, Fmt=*, End=20)
              If (type_in == 0 .and. levcfg > 1) Read(Unit=nread, Fmt=*, End=20)
...
10         Continue
           Rewind(Unit=nread, Err=20)
           If      (type_in == 0 .or. type_in == 3) Then
              Do l=1,5
                 Read(Unit=nread, Fmt=*, End=20)
              End Do
  ...
  If (type_in == 3 .and. natms /= n) Go To 20

! Write summary data

  Write(*,'(1x,2a,i3)') 'INPUT NAME & FORMAT: ', f_in(1:Len_Trim(f_in)),type_in
  Write(*,'(1x,a,6f7.3)') 'DIMENSIONS:',xl,yl,zl,alpha,beta,gamma
  Write(*,'(1x,a,i15,f7.3)') 'SIZE & MAXIMIM CUTOFF RADIUS:',natms/(nx*ny*nz),0.5_wp*Min(wx,wy,wz)
  Write(*,*)
  Write(*,'(1x,2a,i3)') 'OUTPUT NAME & FORMAT: ', f_out(1:Len_Trim(f_out)),type_out
  Write(*,'(1x,a,6f7.3)') 'DIMENSIONS:',Real(nx,wp)*xl,Real(ny,wp)*yl,Real(nz,wp)*zl,alpha,beta,gamma
  Write(*,'(1x,a,i15,f7.3)') 'SIZE & MAXIMIM CUTOFF RADIUS:',natms,0.5_wp*Min(Real(nx,wp)*wx,Real(ny,wp)*wy,Real(nz,wp)*wz)
  Write(*,*)
