...
Contains
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_coms allocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! Get the COM vector
    ...
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_coms deallocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    Allocate (gxx(1:mxlrgd*mxrgd),gyy(1:mxlrgd*mxrgd),gzz(1:mxlrgd*mxrgd), Stat = fail)
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_quench allocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! new atomic velocities in lab frame
    ...
    Deallocate (gxx,gyy,gzz, Stat = fail)
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_quench deallocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    Allocate (gxx(1:mxlrgd*mxrgd),gyy(1:mxlrgd*mxrgd),gzz(1:mxlrgd*mxrgd), Stat = fail)
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_q_ench allocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! new atomic velocities in lab frame
    ...
    Deallocate (gxx,gyy,gzz, Stat = fail)
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_q_ench deallocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    If (Any(fail > 0)) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_setup allocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! of the original representative via global summation.
    ...
    If (fail(1) > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_setup allocation failure 1, node: ', comm%idnode
       Call error(0)
    End If
    ...
    If (fail(1) > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_setup deallocation failure 1, node: ', comm%idnode
       Call error(0)
    End If
    ...
    If (Any(fail > 0)) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_setup deallocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! correct frzsit,dofsit,rgdwgt,rgdwg1 if needed
    ...
    If (fail(1) > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_setup allocation failure 2, node: ', comm%idnode
       Call error(0)
    End If
    lstsit=0
  ...
  ! In case of any refreezing changes refresh the local list of frozen atoms
    ...
    If (fail(1) > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_setup deallocation failure 2, node: ', comm%idnode
       Call error(0)
    End If
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_split_torque allocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! in the plane of rotation wrt the axis of rotation (torque vector)
    ...
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_split_torque deallocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    Allocate (gxx(1:mxlrgd*mxrgd),gyy(1:mxlrgd*mxrgd),gzz(1:mxlrgd*mxrgd), Stat = fail)
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_stress allocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! Symmetrise
    ...
    Deallocate (gxx,gyy,gzz, Stat = fail)
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_stress deallocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_tags allocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! Check for uncompressed units
    ...
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_tags deallocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    Allocate (gxx(1:mxlrgd*mxrgd),gyy(1:mxlrgd*mxrgd),gzz(1:mxlrgd*mxrgd), Stat = fail)
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_widths allocation failure, node: ', comm%idnode
       Call error(0)
    End If
  ...
  ! Check if a RB unit has a diameter > the cutoff
    ...
    Deallocate (gxx,gyy,gzz, Stat = fail)
    If (fail > 0) Then
       Write(nrite,'(/,1x,a,i0)') 'rigid_bodies_widths deallocation failure, node: ', comm%idnode
       Call error(0)
    End If
...
! second order taylor expansion of Exp(tstep*eta)
  ...
  Else ! RBs exist
     ...
     If (fail > 0) Then
        Write(nrite,'(/,1x,a,i0)') 'xscale allocation failure, node: ', comm%idnode
        Call error(0)
     End If
...
! Halo final RB members positions across onto neighbouring domains
     ...
     If (fail > 0) Then
        Write(nrite,'(/,1x,a,i0)') 'xscale deallocation failure, node: ', comm%idnode
        Call error(0)
     End If
