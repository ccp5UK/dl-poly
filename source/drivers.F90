Module drivers
  ...
  Use errors_warnings,      Only: error,&
                                  ...
                                  warning
  Use evb,                  Only: &
                                  evb_check_configs, evb_check_constraints, evb_check_external, &
                                  evb_check_intermolecular, evb_check_intramolecular, &
                                  evb_check_intrinsic, evb_check_vdw, evb_merge_stochastic, &
                                  evb_pes, evb_population, evb_prevent, evb_setzero, evb_type, &
                                  read_evb_settings
      ...
      If (minim%minimise .and. flow%step >= 0 .and. flow%step <= flow%run_steps .and. flow%step <= flow%equil_steps) Then
        If (minim%freq == 0 .and. flow%step == 0) Then
          Call minimise_relax(flow%strict .or. cshell%keyshl == SHELL_RELAXED, &
                              ...
                              minim, rigid, domain, cnfig, files, comm)
  ...
  Subroutine calculate_forces(evbff, cnfig, flow, io, cshell, cons, pmf, stat, plume, pois, bond, angle, dihedral, &
    ...
    Do ff = 1, flow%NUM_FF
      If ((cshell(ff)%megshl > 0 .and. cshell(ff)%keyshl == SHELL_RELAXED) .and. &
        ...
        Call refresh_mappings(cnfig(ff), flow, cshell(ff), cons(ff), pmf(ff), &
                              stat(ff), msd_data(ff), bond(ff), angle(ff), &
                              dihedral(ff), inversion(ff), tether(ff), neigh(ff), sites(ff), &
                              mpoles(ff), rigid(ff), domain(ff), kim_data(ff), &
        ...
        If (minim(ff)%minimise .and. flow%step >= 0 .and. flow%step <= flow%run_steps .and. flow%step <= flow%equil_steps) Then
          If (minim(ff)%freq == 0 .and. flow%step == 0) Then
            Call minimise_relax(flow%strict .or. cshell(ff)%keyshl == SHELL_RELAXED, &
                                ...
                                minim(ff), rigid(ff), domain(ff), cnfig(ff), files, comm)
...
!!!!!!!!!!!!!!!!!  W_STATISTICS_REPORT INCLUSION  !!!!!!!!!!!!!!!!!!!!!!
  ...
  Subroutine write_options(cnfig, io, rsdc, cshell, stat, sites, domain, traj, files, dfcts, &
    ...
    If (traj%ltraj) Then
      Call trajectory_write(flow%restart_key, flow%step, thermo%tstep, flow%time, io, stat%rsd, cnfig, &
                            traj, files, comm)
    End If
    ...
    If (dfcts(1)%ldef) Then
      Call defects_write(flow%restart_key, thermo%ensemble, flow%step, thermo%tstep, &
                         ...
                         neigh, sites, domain, cnfig, files, comm)
  ...
  Subroutine md_vv(cnfig, ttm, io, rsdc, flow, cshell, cons, pmf, stat, thermo, plume, &
                   ...
                   mpoles, ext_field, rigid, domain, seed, traj, kim_data, files, tmr, &
...
!!$#endif
      ...
      If (flow%step > 0 .and. flow%step <= flow%run_steps .and. (.not. flow%newjob)) Then
        ...
        If (thermo(1)%l_tgaus .and. (flow%step <= flow%equil_steps) .and. &
            Mod(flow%step - flow%equil_steps, thermo(1)%freq_tgaus) == 0) Then
          fregauss = .true.
        End If
  ...
  Subroutine replay_history(cnfig, io, rsdc, flow, cshell, cons, pmf, stat, thermo, msd_data, &
                            ...
                            minim, mpoles, ext_field, rigid, electro, domain, seed, traj, kim_data, dfcts, files, &
...
! exit indicator for reading
  ...
  Subroutine replay_historf(cnfig, io, rsdc, flow, cshell, cons, pmf, stat, thermo, plume, &
                            ...
                            fourbody, rdf, minim, mpoles, ext_field, rigid, electro, domain, seed, traj, &
      ...
      If (exout == 0) Then
        ...
        Else
          ...
          Call calculate_forces_for_historf(cnfig, flow, io, cshell, cons, pmf, stat, plume, pois, bond, angle, dihedral, &
                                            inversion, tether, threebody, neigh, sites, vdws, tersoffs, fourbody, rdf, &
                                            minim, mpoles, ext_field, rigid, electro, domain, kim_data, msd_data, &
          ...
          If (dfcts(1)%ldef) Then
            ...
            If (dfcts(2)%ldef) Then
              Call defects_write(flow%restart_key, thermo%ensemble, flow%step, thermo%tstep, flow%time, io, cshell, &
                                 dfcts(2), neigh, sites, domain, cnfig, files, comm)
            End If
          End If
          If (msd_data%l_msd) Then
            Call msd_write(cnfig, flow%restart_key, cnfig%megatm, flow%step, thermo%tstep, flow%time, stat%stpval, &
