...
Contains
  ...
  Subroutine nst_m0_vv(stage, lvar, mndis, mxdis, mxstp, tstep, &
    ...
    Real(Kind=wp)              :: aaa(1:9), bbb(1:9), cell0(1:9), celprp(1:10), chit0, chpzr, &
                                  cint0, eta0(1:9), hstep, mxdr, qstep, rstep, str(1:9), tmp, vir, &
                                  vom(1:3), vzero
...
! uni is the diagonal unit matrix
  ...
  Subroutine nst_m1_vv(stage, lvar, mndis, mxdis, mxstp, tstep, &
    ...
    Logical, Allocatable       :: lstitr(:)
    Real(Kind=wp)              :: aaa(1:9), bbb(1:9), cell0(1:9), celprp(1:10), chit0, chpzr, &
                                  cint0, eta0(1:9), fmx, fmy, fmz, hstep, mxdr, opx, opy, opz, p0, &
                                  p1, p2, p3, qstep, qt0, qt1, qt2, qt3, rot(1:9), rstep, &
                                  str(1:9), tmp, tqx, tqy, tqz, trx, try, trz, vir, vom(1:3), vpx, &
                                  vpy, vpz, vzero, x(1:1), y(1:1), z(1:1)
...
! uni is the diagonal unit matrix
    ...
    If (thermo%newjob_1) Then
      ...
      thermo%unsafe = Any(domain%map_unique > 1)
    End If
